...
#import "../Controllers/GTCFormViewController.h"
#import "../Helpers/NSString+GTCFormAdditions.h"
#import "../Helpers/NSObject+GTCFormAdditions.h"

CGFloat GTCFormUnspecifiedCellHeight = -3.0;
CGFloat GTCFormRowInitialHeight = -2;

@interface GTCFormDescriptor (_GTCFormRowDescriptor)
...
@interface GTCFormSectionDescriptor (_GTCFormRowDescriptor)
...
@interface GTCFormRowDescriptor() <NSCopying>

@property GTCFormBaseCell * cell;
...
@implementation GTCFormRowDescriptor
...
- (instancetype)initWithTag:(NSString *)tag rowType:(NSString *)rowType title:(NSString *)title
{
    self = [super init];
    if (self){
        NSAssert(((![rowType isEqualToString:GTCFormRowDescriptorTypeSelectorPopover] && ![rowType isEqualToString:GTCFormRowDescriptorTypeMultipleSelectorPopover]) || (([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) && ([rowType isEqualToString:GTCFormRowDescriptorTypeSelectorPopover] || [rowType isEqualToString:GTCFormRowDescriptorTypeMultipleSelectorPopover]))), @"You must be running under UIUserInterfaceIdiomPad to use either GTCFormRowDescriptorTypeSelectorPopover or GTCFormRowDescriptorTypeMultipleSelectorPopover rows.");
        _tag = tag;
        _disabled = @NO;
        _hidden = @NO;
        _rowType = rowType;
        _title = title;
        _cellStyle = [rowType isEqualToString:GTCFormRowDescriptorTypeButton] ? UITableViewCellStyleDefault : UITableViewCellStyleValue1;
        ...
        _height = GTCFormRowInitialHeight;
...
{
    if (!_cell){
        id cellClass = self.cellClass ?: [GTCFormViewController cellClassesForRowDescriptorTypes][self.rowType];
        NSAssert(cellClass, @"Not defined GTCFormRowDescriptorType: %@", self.rowType ?: @"");
        ...
        NSAssert([_cell isKindOfClass:[GTCFormBaseCell class]], @"UITableViewCell must extend from GTCFormBaseCell");
...
-(GTCFormAction *)action
{
    if (!_action){
        _action = [[GTCFormAction alloc] init];
    }
    return _action;
}

-(void)setAction:(GTCFormAction *)action
{
    _action = action;
}
...
-(CGFloat)height
{
    if (_height == GTCFormRowInitialHeight){
        if ([[self.cell class] respondsToSelector:@selector(formDescriptorCellHeightForRowDescriptor:)]){
            ...
            _height = GTCFormUnspecifiedCellHeight;
        }
    }
    return _height;
}
...
{
    GTCFormRowDescriptor * rowDescriptorCopy = [GTCFormRowDescriptor formRowDescriptorWithTag:nil rowType:[self.rowType copy] title:[self.title copy]];
...
- (void)addValidator:(id<GTCFormValidatorProtocol>)validator
{
    if (validator == nil || ![validator conformsToProtocol:@protocol(GTCFormValidatorProtocol)])
        return;
...
- (void)removeValidator:(id<GTCFormValidatorProtocol>)validator
{
    if (validator == nil|| ![validator conformsToProtocol:@protocol(GTCFormValidatorProtocol)])
        return;
...
- (GTCFormValidationStatus *)doValidation
{
    GTCFormValidationStatus *valStatus = nil;

    if (self.required) {
        // do required validation here
        if ([self valueIsEmpty]) {
            valStatus = [GTCFormValidationStatus formValidationStatusWithMsg:@"" status:NO rowDescriptor:self];
            NSString *msg = nil;
    ...
    // custom validator
    for(id<GTCFormValidatorProtocol> v in self.validators) {
        if ([v conformsToProtocol:@protocol(GTCFormValidatorProtocol)]) {
            GTCFormValidationStatus *vStatus = [v isValid:self];
            // fail validation
...
@implementation GTCFormLeftRightSelectorOption


+(GTCFormLeftRightSelectorOption *)formLeftRightSelectorOptionWithLeftValue:(id)leftValue
                                                          httpParameterKey:(NSString *)httpParameterKey
                                                              rightOptions:(NSArray *)rightOptions;
{
    return [[GTCFormLeftRightSelectorOption alloc] initWithLeftValue:leftValue
...
@implementation GTCFormAction

- (instancetype)init
{
    self = [super init];
    if (self) {
        _viewControllerPresentationMode = GTCFormPresentationModeDefault;
    }
    return self;
}
...
- (id)copyWithZone:(NSZone *)zone
{
    GTCFormAction * actionCopy = [[GTCFormAction alloc] init];
...
-(void)setFormBlock:(void (^)(GTCFormRowDescriptor *))formBlock
{
    _formSegueClass = nil;
    _formSegueIdentifier = nil;
    _formSelector = nil;
