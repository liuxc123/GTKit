...
/// KVO context pointer identifying changes in GTCItemBarItem properties.
...
@interface GTCItemBarFlowLayout : UICollectionViewFlowLayout
@end

#pragma mark -

@interface GTCItemBar () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@end

@implementation GTCItemBar {
    // Collection view and layout for items.
    UICollectionView *_collectionView;
    UICollectionViewFlowLayout *_flowLayout;

    /// Indicator layered under the active item.
    GTCTabBarIndicatorView *_selectionIndicator;
...
- (void)commonItemBarInit {
    _alignment = GTCItemBarAlignmentLeading;
    _style = [[GTCItemBarStyle alloc] init];
    _items = @[];
    ...
    [collectionView registerClass:[GTCItemBarCell class] forCellWithReuseIdentifier:kItemReuseID];

    _collectionView = collectionView;
    [self addSubview:_collectionView];

    // Configure the selection indicator view.
    _selectionIndicator = [[GTCTabBarIndicatorView alloc] initWithFrame:CGRectZero];
...
- (void)applyStyle:(GTCItemBarStyle *)style {
...
- (void)setAlignment:(GTCItemBarAlignment)alignment {
...
- (id)accessibilityElementForItem:(UITabBarItem *)item {
    ...
    if (index != NSNotFound) {
        ...
        if ([visibleItems containsObject:indexPath]) {
            GTCItemBarCell *itemCell =
            (GTCItemBarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
            return itemCell;
        }
    }
    return nil;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kItemPropertyContext) {
        // GTCItemBarItem change, must be on the main thread.
        ...
        if (itemIndex != NSNotFound) {
            ...
            if (cell) {
                NSAssert([cell isKindOfClass:[GTCItemBarCell class]], @"All cells must be GTCItemBarCell");
                GTCItemBarCell *itemCell = (GTCItemBarCell *)cell;
...
shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_collectionView == collectionView) {
        ...
        id<GTCItemBarDelegate> delegate = self.delegate;
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    size = [GTCItemBarCell sizeThatFits:size
                    horizontalSizeClass:[self horizontalSizeClass]
                                   item:item
                                  style:_style];

    // Divide justified items evenly across the view.
    if (_alignment == GTCItemBarAlignmentJustified) {
...
- (UICollectionViewFlowLayout *)generatedFlowLayout {
    UICollectionViewFlowLayout *flowLayout = [[GTCItemBarFlowLayout alloc] init];
...
- (void)didSelectItemAtIndex:(NSInteger)index animateTransition:(BOOL)animate {
    ...
    if (animate) {
        ...
        [CAMediaTimingFunction gtc_functionWithType:GTCAnimationTimingFunctionEaseInOut];
...
- (void)updateSelectionIndicatorToIndex:(NSInteger)index {
    ...
    UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:indexPath];
    if ([cell isKindOfClass:[GTCItemBarCell class]]) {
        GTCItemBarCell *itemBarCell = (GTCItemBarCell *)cell;
    ...
    UITabBarItem *item = [self itemAtIndexPath:indexPath];
    GTCTabBarPrivateIndicatorContext *context =
    [[GTCTabBarPrivateIndicatorContext alloc] initWithItem:item
                                                    bounds:selectionIndicatorBounds
                                              contentFrame:contentFrame];

    // Ask the template for attributes.
    id<GTCTabBarIndicatorTemplate> template = _style.selectionIndicatorTemplate;
    GTCTabBarIndicatorAttributes *indicatorAttributes =
...
- (void)updateScrollProperties {
    _collectionView.alwaysBounceHorizontal = (_alignment != GTCItemBarAlignmentJustified);
}
...
- (void)configureCell:(GTCItemBarCell *)cell {
...
#pragma mark - GTCItemBarFlowLayout

@implementation GTCItemBarFlowLayout {
...
(UICollectionViewLayoutAttributes *)attributes {
    ...
    newAttributes.frame = GTFRectFlippedHorizontally(itemFrame, CGRectGetWidth(collectionBounds));
...
- (BOOL)shouldEnforceRightToLeftLayout {
    ...
    self.collectionView.gtf_effectiveUserInterfaceLayoutDirection == rtl;
}
...
- (BOOL)shouldRelayoutAttributesForRTL {
    ...
    // always be the case for GTCItemBar, but check for safety.
