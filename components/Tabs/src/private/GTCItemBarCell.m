...
#import "GTCItemBarCell+Private.h"

#import <GTFInternationalization/GTFInternationalization.h>

#import "GTCItemBarStringConstants.h"
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _inkTouchController = [[GTCInkTouchController alloc] initWithView:self];
...
+ (CGSize)sizeThatFits:(CGSize)size
   horizontalSizeClass:(UIUserInterfaceSizeClass)sizeClass
    ...
    if (style.shouldDisplayBadge && badge.length > 0) {
        UIFont *badgeFont = [[GTCTypography fontLoader] regularFontOfSize:kBadgeFontSize];
...
- (void)layoutSubviews {
    ...
    _imageView.center = GTCRoundCenterWithBoundsAndScale(imageCenter, _imageView.bounds, scale);

    _badgeLabel.bounds = GTCRectAlignToScale(badgeBounds, scale);
    _badgeLabel.center = GTCRoundCenterWithBoundsAndScale(badgeCenter, _badgeLabel.bounds, scale);

    self.titleLabel.bounds = GTCRectAlignToScale(titleBounds, scale);
    self.titleLabel.center = GTCRoundCenterWithBoundsAndScale(titleCenter,
...
- (nullable NSString *)accessibilityLabel {
    ...
    NSString *tabLabel =
    [[self class] localizedStringWithKey:kGTCItemBarStringKeyAccessibilityTabElementLabel];
    if (tabLabel) {
        [labelComponents addObject:tabLabel];
    }

    NSString *positionFormat =
    [[self class] localizedStringWithKey:kGTCItemBarStringKeyAccessibilityTabPositionFormat];
    if (positionFormat) {
...
+ (NSString *)displayedTitleForTitle:(NSString *)title style:(GTCItemBarStyle *)style {
...
- (void)updateSubviews {
    ...
    if (_style.shouldDisplayBadge) {
        if (!_badgeLabel) {
            ...
            _badgeLabel.font = [[GTCTypography fontLoader] regularFontOfSize:kBadgeFontSize];
...
- (void)updateTransformsAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [CAMediaTimingFunction gtc_functionWithType:GTCAnimationTimingFunctionTranslate];
...
- (void)updateInk {
    GTCInkView *inkView = _inkTouchController.defaultInkView;
    inkView.inkColor = _style.inkColor;
    inkView.inkStyle = _style.inkStyle;
    inkView.usesLegacyInkRipple = NO;
    inkView.clipsToBounds = (inkView.inkStyle == GTCInkStyleBounded) ? YES : NO;
}
