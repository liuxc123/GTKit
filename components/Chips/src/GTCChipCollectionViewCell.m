...
static NSString *const GTCChipCollectionViewCellChipViewKey =
@"GTCChipCollectionViewCellChipViewKey";

@implementation GTCChipCollectionViewCell
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        _chipView = [aDecoder decodeObjectForKey:GTCChipCollectionViewCellChipViewKey];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:self.chipView forKey:GTCChipCollectionViewCellChipViewKey];
}

- (void)setBounds:(CGRect)bounds {
    [super setBounds:bounds];

    // We update selected state here in the event that the GTCChipView will resize upon selection.
...
- (GTCChipView *)createChipView {
    return [[GTCChipView alloc] init];
}
...
- (void)setSelected:(BOOL)selected {
    ...
    if (self.alwaysAnimateResize && [_chipView willChangeSizeWithSelectedValue:selected]) {
        // Since GTCChipView can resize when it is selected, if a resize will occur we need to delay our
