...
#import "private/GTCChipView+Private.h"
...
static const GTCFontTextStyle kTitleTextStyle = GTCFontTextStyleBody2;

static const CGSize kGTCChipMinimumSizeDefault = (CGSize){(CGFloat)0, (CGFloat)32};

// Creates a UIColor from a 24-bit RGB color encoded as an integer.
static inline UIColor *GTCColorFromRGB(uint32_t rgbValue) {
...
static inline UIColor *GTCColorDarken(UIColor *color, CGFloat percent) {
    CGFloat hue;
    CGFloat saturation;
    CGFloat brightness;
    CGFloat alpha;
...
static inline UIColor *GTCColorLighten(UIColor *color, CGFloat percent) {
    return GTCColorDarken(color, -percent);
}

// TODO(samnm): Pull background color from GTCPalette
static const uint32_t GTCChipBackgroundColor = 0xEBEBEB;
static const CGFloat GTCChipSelectedDarkenPercent = 0.16f;
static const CGFloat GTCChipDisabledLightenPercent = 0.38f;
static const CGFloat GTCChipTitleColorWhite = 0.13f;
static const CGFloat GTCChipTitleColorDisabledLightenPercent = 0.38f;

static const UIEdgeInsets GTCChipContentPadding = {4, 4, 4, 4};
static const UIEdgeInsets GTCChipImagePadding = {0, 0, 0, 0};
static const UIEdgeInsets GTCChipTitlePadding = {3, 8, 4, 8};
static const UIEdgeInsets GTCChipAccessoryPadding = {0, 0, 0, 0};
...
static CGRect CGRectVerticallyCentered(CGRect rect,
    ...
    yValue = GTCRound(yValue * pixelScale) / pixelScale;
    return CGRectOffset(rect, 0, yValue);
}

static inline CGRect GTCChipBuildFrame(UIEdgeInsets insets,
                                       CGSize size,
                                       CGFloat xOffset,
                                       CGFloat chipHeight,
                                       CGFloat pixelScale) {
...
- (void)commonGTCChipViewInit {
    _minimumSize = kGTCChipMinimumSizeDefault;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        if (!_backgroundColors) {
            ...
            UIColor *normal = GTCColorFromRGB(GTCChipBackgroundColor);
            UIColor *disabled = GTCColorLighten(normal, GTCChipDisabledLightenPercent);
            UIColor *selected = GTCColorDarken(normal, GTCChipSelectedDarkenPercent);
        ...
        _elevations[@(UIControlStateHighlighted)] = @(GTCShadowElevationRaisedButtonPressed);
        _elevations[@(UIControlStateHighlighted | UIControlStateSelected)] =
        @(GTCShadowElevationRaisedButtonPressed);

        _inkColors = [NSMutableDictionary dictionary];

        UIColor *titleColor = [UIColor colorWithWhite:GTCChipTitleColorWhite alpha:1.0f];
        ...
        GTCColorLighten(titleColor, GTCChipTitleColorDisabledLightenPercent);
        ...
        if ([GTCTypography.fontLoader isKindOfClass:[GTCSystemFontLoader class]]) {
            _titleLabel.font = [UIFont gtc_standardFontForMaterialTextStyle:kTitleTextStyle];
        } else {
            // There is a custom font loader, retrieve the font from it.
            _titleLabel.font = [GTCTypography buttonFont];
        }
        _titleLabel.textAlignment = NSTextAlignmentCenter;
        [self addSubview:_titleLabel];

        _contentPadding = GTCChipContentPadding;
        _imagePadding = GTCChipImagePadding;
        _titlePadding = GTCChipTitlePadding;
        _accessoryPadding = GTCChipAccessoryPadding;
...
- (void)updateTitleFont {
    ...
    if (customTitleFont) {
        ...
        if (_gtc_adjustsFontForContentSizeCategory) {
            ...
            [customTitleFont gtc_fontSizedForMaterialTextStyle:kTitleTextStyle
        ...
        if (_gtc_adjustsFontForContentSizeCategory) {
            ...
            if ([GTCTypography.fontLoader isKindOfClass:[GTCSystemFontLoader class]]) {
                _titleLabel.font = [UIFont gtc_preferredFontForMaterialTextStyle:kTitleTextStyle];
            } else {
                // There is a custom font loader, retrieve the font and scale it.
                UIFont *customTypographyFont = [GTCTypography buttonFont];
                _titleLabel.font =
                [customTypographyFont gtc_fontSizedForMaterialTextStyle:kTitleTextStyle
                                                   scaledForDynamicType:_gtc_adjustsFontForContentSizeCategory];
            }
        } else {
            // If we are using the default (system) font loader, retrieve the
...
- (void)layoutSubviews {
    ...
    if (self.gtf_effectiveUserInterfaceLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft) {
        for (UIView *subview in self.subviews) {
            CGRect flippedRect =
            GTFRectFlippedHorizontally(subview.frame, CGRectGetWidth(self.bounds));
...
- (CGRect)frameForImageView:(UIImageView *)imageView visible:(BOOL)visible {
    ...
    if (visible) {
        ...
        frame = GTCChipBuildFrame(_imagePadding,
                                  selectedSize,
...
- (CGRect)accessoryViewFrame {
    ...
    return GTCChipBuildFrame(_accessoryPadding,
                             size,
                             xOffset,
                             CGRectGetHeight(self.frame),
                             self.pixelScale);
...
- (CGRect)titleLabelFrame {
    ...
    return GTCChipBuildFrame(_titlePadding,
                             titleSize,
                             imageRightEdge,
                             CGRectGetHeight(self.frame),
                             self.pixelScale);
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    return GTCSizeCeilWithScale(chipSize, self.pixelScale);
}
...
- (void)touchDragEnter:(__unused GTCChipView *)button forEvent:(UIEvent *)event {
    [self startTouchBeganAnimationAtPoint:[self locationFromTouches:event.allTouches]];
}

- (void)touchDragExit:(__unused GTCChipView *)button forEvent:(UIEvent *)event {
...
@implementation GTCChipView (Private)
...
- (void)startTouchBeganAnimationAtPoint:(CGPoint)point {
    ...
    (CGFloat)(GTCHypot(size.height, size.width + widthDiff) / 2 + 10.f + widthDiff / 2);
