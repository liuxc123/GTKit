//
//  ButtonsTypicalUseSupplemental.m
//  GTCatalog
//
...
@implementation ButtonsTypicalUseExampleViewController (GTCatalog)
...
- (UILabel *)addLabelWithText:(NSString *)text {
    ...
    label.alpha = [GTCTypography captionFontOpacity];
    [label sizeToFit];
...
- (void)viewDidLayoutSubviews {
    ...
    if (lastButtonMaxY > CGRectGetMaxY(contentBounds)) {
        ...
        if (((GTCButton *)self.buttons[splitIndex - 1]).enabled) {
            splitIndex++;
        }
...
- (void)layoutButtonsInRange:(NSRange)range around:(CGFloat)centerX {
    ...
    for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
        GTCButton *button = self.buttons[i];
    ...
    GTCButton *lastButton = self.buttons[NSMaxRange(range) - 1];
    ...
    for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
        GTCButton *button = self.buttons[i];
        UILabel *label = self.labels[i];
        button.center = GTCRoundCenterWithBoundsAndScale(CGPointMake(button.center.x,
        ...
        CGRect alignedFrame = GTCRectAlignToScale(
