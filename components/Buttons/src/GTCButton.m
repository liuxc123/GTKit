...
static const CGFloat GTCButtonMinimumTouchTargetHeight = 48;
static const CGFloat GTCButtonMinimumTouchTargetWidth = 48;
static const CGFloat GTCButtonDefaultCornerRadius = 2.0;

static const NSTimeInterval GTCButtonAnimationDuration = 0.2;

// https://material.io/go/design-buttons#buttons-main-buttons
static const CGFloat GTCButtonDisabledAlpha = 0.12f;

// Blue 500 from https://material.io/go/design-color-theming#color-color-palette .
static const uint32_t GTCButtonDefaultBackgroundColor = 0x191919;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.titleLabel.font = [GTCTypography buttonFont];

        [self commonGTCButtonInit];
...
- (void)commonGTCButtonInit {
    _disabledAlpha = GTCButtonDisabledAlpha;
    ...
    if (!_backgroundColors) {
        ...
        _backgroundColors[@(UIControlStateNormal)] = GTCColorFromRGB(GTCButtonDefaultBackgroundColor);
    }
...
- (void)layoutSubviews {
    ...
    if (_inkView.inkStyle == GTCInkStyleUnbounded && _inkView.usesLegacyInkRipple) {
    ...
    self.titleLabel.frame = GTCRectAlignToScale(self.titleLabel.frame, [UIScreen mainScreen].scale);
}
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (width < GTCButtonMinimumTouchTargetWidth || height < GTCButtonMinimumTouchTargetHeight) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            NSLog(
                  @"Button touch target does not meet minimum size guidlines of (%0.f, %0.f). Button: %@, "
                  @"Touch Target: %@",
                  GTCButtonMinimumTouchTargetWidth,
                  GTCButtonMinimumTouchTargetHeight,
                  [self description],
...
- (void)animateButtonToHeightForState:(UIControlState)state {
    ...
    [CATransaction begin];
    [CATransaction setAnimationDuration:GTCButtonAnimationDuration];
...
- (void)setElevation:(CGFloat)elevation forState:(UIControlState)state {
    ...
    GTCShadowElevation newElevation = [self elevationForState:self.state];
    // If no change to the current elevation, don't perform updates
    if (GTCCGFloatEqual(newElevation, self.layer.elevation)) {
        return;
    }
    self.layer.elevation = newElevation;
    ...
    // TODO(ajsecord): Move to GTCFlatButton and update this comment.
...
- (BOOL)isDarkColor:(UIColor *)color {
    // TODO: have a components/private/ColorCalculations/GTCColorCalculations.h|m
...
- (void)touchDragEnter:(__unused GTCButton *)button forEvent:(UIEvent *)event {
    [self handleBeginTouches:event.allTouches];
}

- (void)touchDragExit:(__unused GTCButton *)button forEvent:(UIEvent *)event {
...
- (BOOL)shouldHaveOpaqueBackground {
    BOOL isFlatButton = GTCCGFloatIsExactlyZero([self elevationForState:UIControlStateNormal]);
    return !isFlatButton;
}
...
- (void)updateAlphaAndBackgroundColorAnimated:(BOOL)animated {
    ...
    if (animated) {
        [UIView animateWithDuration:GTCButtonAnimationDuration animations:animations];
    } else {
        animations();
    }
}
...
- (void)updateTitleFont {
    ...
    if (!font) {
        // TODO(#2709): Have a single source of truth for fonts
        // Migrate to [UIFont standardFont] when possible
        font = [GTCTypography buttonFont];
    }

    if (_gtc_adjustsFontForContentSizeCategory) {
        // Dynamic type is enabled so apply scaling
        font = [font gtc_fontSizedForMaterialTextStyle:GTCFontTextStyleButton
