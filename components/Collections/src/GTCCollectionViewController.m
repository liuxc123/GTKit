...
#import "private/GTCCollectionStringResources.h"
#import "private/GTCCollectionViewEditor.h"
#import "private/GTCCollectionViewStyler.h"

#include <tgmath.h>

NSString *const GTCCollectionInfoBarKindHeader = @"GTCCollectionInfoBarKindHeader";
NSString *const GTCCollectionInfoBarKindFooter = @"GTCCollectionInfoBarKindFooter";

@interface GTCCollectionViewController () <GTCCollectionInfoBarViewDelegate,
GTCInkTouchControllerDelegate>
@property(nonatomic, assign) BOOL currentlyActiveInk;
@end


@implementation GTCCollectionViewController{
    GTCInkTouchController *_inkTouchController;
    GTCCollectionInfoBarView *_headerInfoBar;
    GTCCollectionInfoBarView *_footerInfoBar;
...
- (instancetype)init {
    GTCCollectionViewFlowLayout *defaultLayout = [[GTCCollectionViewFlowLayout alloc] init];
...
- (instancetype)initWithNibName:(nullable NSString *)nibNameOrNil
    ...
    if (self != nil) {
        ...
        if (!_collectionViewLayout) {
            _collectionViewLayout = [[GTCCollectionViewFlowLayout alloc] init];
        }
    }
...
- (void)viewDidLoad {
    ...
    _styler = [[GTCCollectionViewStyler alloc] initWithCollectionView:self.collectionView];
    _styler.delegate = self;

    _editor = [[GTCCollectionViewEditor alloc] initWithCollectionView:self.collectionView];
    _editor.delegate = self;

    // Set up ink touch controller.
    _inkTouchController = [[GTCInkTouchController alloc] initWithView:self.collectionView];
    _inkTouchController.delegate = self;

    // Register our supplementary header and footer
    NSString *classIdentifier = NSStringFromClass([GTCCollectionInfoBarView class]);
    NSString *headerKind = GTCCollectionInfoBarKindHeader;
    NSString *footerKind = GTCCollectionInfoBarKindFooter;
    [self.collectionView registerClass:[GTCCollectionInfoBarView class]
...
- (void)setCollectionView:(__kindof UICollectionView *)collectionView {
    ...
    _editor = [[GTCCollectionViewEditor alloc] initWithCollectionView:collectionView];
    _editor.delegate = self;
    _inkTouchController = [[GTCInkTouchController alloc] initWithView:collectionView];
    _inkTouchController.delegate = self;
}

#pragma mark - <GTCCollectionInfoBarViewDelegate>

- (void)updateControllerWithInfoBar:(GTCCollectionInfoBarView *)infoBar {
    // Updates info bar styling for header/footer.
    if ([infoBar.kind isEqualToString:GTCCollectionInfoBarKindHeader]) {
        _headerInfoBar = infoBar;
        _headerInfoBar.message = GTCCollectionStringResources(infoBarGestureHintString);
        _headerInfoBar.style = GTCCollectionInfoBarViewStyleHUD;
        [self updateHeaderInfoBarIfNecessary];
    } else if ([infoBar.kind isEqualToString:GTCCollectionInfoBarKindFooter]) {
        _footerInfoBar = infoBar;
        _footerInfoBar.message = GTCCollectionStringResources(deleteButtonString);
        _footerInfoBar.style = GTCCollectionInfoBarViewStyleActionable;
        [self updateFooterInfoBarIfNecessary];
    }
}

- (void)didTapInfoBar:(GTCCollectionInfoBarView *)infoBar {
    if ([infoBar isEqual:_footerInfoBar]) {
        [self deleteIndexPaths:self.collectionView.indexPathsForSelectedItems];
    }
}

- (void)infoBar:(GTCCollectionInfoBarView *)infoBar
willShowAnimated:(__unused BOOL)animated
willAutoDismiss:(__unused BOOL)willAutoDismiss {
    if ([infoBar.kind isEqualToString:GTCCollectionInfoBarKindFooter]) {
        [self updateContentWithBottomInset:GTCCollectionInfoBarFooterHeight];
    }
}
...
- (GTCCollectionViewCellStyle)collectionView:(__unused UICollectionView *)collectionView
...
minimumLineSpacingForSectionAtIndex:(__unused NSInteger)section {
    if ([collectionViewLayout isKindOfClass:[UICollectionViewFlowLayout class]]) {
        if (_styler.cellLayoutType == GTCCollectionViewCellLayoutTypeGrid) {
...
- (CGSize)sizeWithAttribute:(UICollectionViewLayoutAttributes *)attr
    ...
    CGFloat height = GTCCellDefaultOneLineHeight;
...
- (UIEdgeInsets)insetsAtSectionIndex:(NSInteger)section
    ...
    GTCCollectionViewCellStyle cellStyle = [_styler cellStyleAtSectionIndex:section];
...
#pragma mark - <GTCInkTouchControllerDelegate>

- (BOOL)inkTouchController:(__unused GTCInkTouchController *)inkTouchController
...
- (GTCInkView *)inkTouchController:(GTCInkTouchController *)inkTouchController
    ...
    if ([cell isKindOfClass:[GTCCollectionViewCell class]]) {
        GTCCollectionViewCell *inkCell = (GTCCollectionViewCell *)cell;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    if ([kind isEqualToString:GTCCollectionInfoBarKindHeader] ||
        [kind isEqualToString:GTCCollectionInfoBarKindFooter]) {
        NSString *identifier = NSStringFromClass([GTCCollectionInfoBarView class]);
        ...
        // Update info bar.
        if ([supplementaryView isKindOfClass:[GTCCollectionInfoBarView class]]) {
            GTCCollectionInfoBarView *infoBarView = (GTCCollectionInfoBarView *)supplementaryView;
...
- (void)updateContentWithBottomInset:(CGFloat)inset {
    ...
    [UIView animateWithDuration:GTCCollectionInfoBarAnimationDuration
                     animations:^{
