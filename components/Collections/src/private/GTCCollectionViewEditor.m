//
//  GTCCollectionViewEditor.m
//  GTCatalog
//
//  Created by liuxc on 2018/8/29.
//

#import "GTCCollectionViewEditor.h"
...
static const CGFloat kGTCAutoscrollPanningBuffer = 60.0f;

// Distance collection view should offset during autoscroll.
static const CGFloat kGTCAutoscrollPanningOffset = 10.0f;

/** Autoscroll panning direction. */
typedef NS_ENUM(NSInteger, GTCAutoscrollPanningDirection) {
    kGTCAutoscrollPanningDirectionNone,
    kGTCAutoscrollPanningDirectionUp,
    kGTCAutoscrollPanningDirectionDown
};

/** A view that uses an GTCShadowLayer as its sublayer. */
...
@interface GTCCollectionViewEditor () <UIGestureRecognizerDelegate>
@end

#if defined(__IPHONE_10_0) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_10_0)
@interface GTCCollectionViewEditor () <CAAnimationDelegate>
@end
#endif

@implementation GTCCollectionViewEditor{
    ...
    GTCAutoscrollPanningDirection _autoscrollPanningDirection;
}
...
- (void)applyLayerShadowing:(__unused CALayer *)layer {
    GTCShadowLayer *shadowLayer = (GTCShadowLayer *)_cellSnapshot.layer;
...
- (void)panToReorderWithRecognizer:(UIPanGestureRecognizer *)recognizer {
    if (recognizer.state == UIGestureRecognizerStateChanged) {
        ...
        CGFloat buffer = kGTCAutoscrollPanningBuffer;
        if (_selectedCellLocation.y < CGRectGetMinY(self.collectionView.bounds) + buffer) {
            [self startAutoscroll];
            _autoscrollPanningDirection = kGTCAutoscrollPanningDirectionUp;
        } else if (_selectedCellLocation.y > (CGRectGetMaxY(self.collectionView.bounds) - buffer)) {
            [self startAutoscroll];
            _autoscrollPanningDirection = kGTCAutoscrollPanningDirectionDown;
        } else {
            [self stopAutoscroll];
        }
...
- (void)stopAutoscroll {
    if (_autoscrollTimer) {
        ...
        _autoscrollPanningDirection = kGTCAutoscrollPanningDirectionNone;
    }
}
...
- (void)autoscroll:(__unused CADisplayLink *)sender {
    ...
    BOOL isPanningDown = _autoscrollPanningDirection == kGTCAutoscrollPanningDirectionDown;
    CGFloat yOffset = kGTCAutoscrollPanningOffset * (isPanningDown ? 1 : -1);
