//
//  GTCDialogPresentationController.m
//  GTCatalog
//
...
#import "private/GTCDialogShadowedView.h"

static CGFloat GTCDialogMinimumWidth = 280.0f;
...
static UIEdgeInsets GTCDialogEdgeInsets = {24, 20, 24, 20};

@interface GTCDialogPresentationController ()
...
@property(nonatomic) GTCDialogShadowedView *trackingView;

@end

@implementation GTCDialogPresentationController {
...
- (instancetype)initWithPresentedViewController:(UIViewController *)presentedViewController
    ...
    if (self) {
        ...
        _trackingView = [[GTCDialogShadowedView alloc] init];
...
- (CGRect)frameOfPresentedViewInContainerView {
    ...
    containerSafeAreaInsets.top = MAX(containerSafeAreaInsets.top, GTCDialogEdgeInsets.top);
    containerSafeAreaInsets.left = MAX(containerSafeAreaInsets.left, GTCDialogEdgeInsets.left);
    containerSafeAreaInsets.right = MAX(containerSafeAreaInsets.right, GTCDialogEdgeInsets.right);
    containerSafeAreaInsets.bottom = MAX(containerSafeAreaInsets.bottom, GTCDialogEdgeInsets.bottom);

    // Take into account a visible keyboard
    CGFloat keyboardHeight = [GTCKeyboardWatcher sharedKeyboardWatcher].visibleKeyboardHeight;
...
/**
 ...
 GTCDialogPresentationController does not cover the presenting view controller's content entirely
...
- (CGSize)sizeForChildContentContainer:(id<UIContentContainer>)container
    ...
    if (!CGSizeEqualToSize(preferredContentSize, CGSizeZero)) {
        ...
        // If the targetSize.width is greater than 0.0 it must be at least GTCDialogMinimumWidth.
        if (0.0f < targetSize.width && targetSize.width < GTCDialogMinimumWidth) {
            targetSize.width = GTCDialogMinimumWidth;
        }
...
- (void)registerKeyboardNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardWatcherHandler:)
                                                 name:GTCKeyboardWatcherKeyboardWillShowNotification
                                               object:nil];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardWatcherHandler:)
                                                 name:GTCKeyboardWatcherKeyboardWillHideNotification
                                               object:nil];

    [[NSNotificationCenter defaultCenter]
     addObserver:self
     selector:@selector(keyboardWatcherHandler:)
     name:GTCKeyboardWatcherKeyboardWillChangeFrameNotification
     object:nil];
}
...
- (void)keyboardWatcherHandler:(NSNotification *)aNotification {
    ...
    [GTCKeyboardWatcher animationDurationFromKeyboardNotification:aNotification];

    UIViewAnimationOptions animationCurveOption =
    [GTCKeyboardWatcher animationCurveOptionFromKeyboardNotification:aNotification];
