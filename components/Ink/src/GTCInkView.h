...
@protocol GTCInkViewDelegate;

/** Completion block signature for all ink animations. */
typedef void (^GTCInkCompletionBlock)(void);

/** Ink styles. */
typedef NS_ENUM(NSInteger, GTCInkStyle) {
    GTCInkStyleBounded,  /** Ink is clipped to the view's bounds. */
    GTCInkStyleUnbounded /** Ink is not clipped to the view's bounds. */
};
 ...
 */
@interface GTCInkView : UIView

/**
 Ink view animation delegate. Clients set this delegate to receive updates when ink animations
 start and end.
 */
@property(nonatomic, weak, nullable) id<GTCInkViewDelegate> animationDelegate;

/**
 The style of ink for this view. Defaults to GTCInkStyleBounded.

 Changes only affect subsequent animations, not animations in progress.
 */
@property(nonatomic, assign) GTCInkStyle inkStyle;
...
/**
 ...
 is used. This value is ignored if @c inkStyle is set to |GTCInkStyleBounded|.
...
- (void)startTouchBeganAnimationAtPoint:(CGPoint)point
                             completion:(nullable GTCInkCompletionBlock)completionBlock;
...
/**
 Enumerates the given view's subviews for an instance of GTCInkView and returns it if found, or
 creates and adds a new instance of GTCInkView if not.
 ...
 This method can be used in your GTCInkTouchController delegate's
 -inkTouchController:inkViewAtTouchLocation; implementation.
 */
+ (nonnull GTCInkView *)injectedInkViewForView:(nonnull UIView *)view;

@end

/**
 Delegate protocol for GTCInkView. Clients may implement this protocol to receive updates when ink
 layer start and end.
 */
@protocol GTCInkViewDelegate <NSObject>
...
/**
 ...
 @param inkView The GTCInkView that starts animating.
 */
- (void)inkAnimationDidStart:(nonnull GTCInkView *)inkView;

/**
 Called when the ink ripple animation ends.

 @param inkView The GTCInkView that ends animating.
 */
- (void)inkAnimationDidEnd:(nonnull GTCInkView *)inkView;
