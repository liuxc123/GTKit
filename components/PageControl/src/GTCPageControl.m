...
#import "private/GTCPageControlIndicator.h"
#import "private/GTCPageControlTrackLayer.h"
#import "private/GTPageControlStrings.h"
#import "private/GTPageControlStrings_table.h"

#include <tgmath.h>

// The Bundle for string resources.
static NSString *const kGTPageControlBundle = @"GTPageControl.bundle";

// The keypath for the content offset of a scrollview.
static NSString *const kGTPageControlScrollViewContentOffset = @"bounds.origin";
...
@implementation GTCPageControl {
    UIView *_containerView;
    NSMutableArray<GTCPageControlIndicator *> *_indicators;
    NSMutableArray<NSValue *> *_indicatorPositions;
    GTCPageControlIndicator *_animatedIndicator;
    GTCPageControlTrackLayer *_trackLayer;
...
- (void)commonGTCPageControlInit {
    ...
    _trackLayer = [[GTCPageControlTrackLayer alloc] initWithRadius:radius];
...
- (void)layoutSubviews {
    ...
    self.hidden = NO;
    for (GTCPageControlIndicator *indicator in _indicators) {
    ...
    //  if ([self class] == [GTCPageControl class]) {
    //    [self GTC_flipViewForRTL];
    //  }
}
...
- (CGSize)sizeThatFits:(__unused CGSize)size {
    return [GTCPageControl sizeForNumberOfPages:_numberOfPages];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([scrollView.layer.animationKeys containsObject:kGTPageControlScrollViewContentOffset]) {
        CAAnimation *animation =
        [scrollView.layer animationForKey:kGTPageControlScrollViewContentOffset];
...
- (void)revealIndicatorsReversed:(BOOL)reversed {
    ...
    NSArray<GTCPageControlIndicator *> *indicators =
    reversed ? [[_indicators reverseObjectEnumerator] allObjects] : _indicators;
    NSInteger count = 0;
    for (GTCPageControlIndicator *indicator in indicators) {
...
- (void)updateCurrentPageDisplay {
    ...
    if (_defersCurrentPageDisplay && [self isPageIndexValid:_currentPage]) {
        ...
        for (int i = 0; i < _numberOfPages; i++) {
            GTCPageControlIndicator *indicator = _indicators[i];
...
- (void)resetControl {
    ...
    for (int i = 0; i < _numberOfPages; i++) {
        ...
        GTCPageControlIndicator *indicator =
        [[GTCPageControlIndicator alloc] initWithCenter:center radius:radius];
    ...
    CGSize controlSize = [GTCPageControl sizeForNumberOfPages:_numberOfPages];
    ...
    _animatedIndicator = [[GTCPageControlIndicator alloc] initWithCenter:center radius:radius];
    [_animatedIndicator updateIndicatorTransformX:point.x - kPageControlIndicatorRadius];
...
+ (NSString *)pageControlAccessibilityLabelWithPage:(NSInteger)currentPage
    ...
    NSString *key = kGTPageControlStringTable[kStr_GTPageControlAccessibilityLabel];
    NSString *localizedString = NSLocalizedStringFromTableInBundle(
                                                                   key, kGTPageControlStringsTableName, [self bundle], @"page {number} of {total number}");
...
+ (NSBundle *)bundle {
    ...
    dispatch_once(&onceToken, ^{
        bundle = [NSBundle bundleWithPath:[self bundlePathWithName:kGTPageControlBundle]];
    });
...
+ (NSString *)bundlePathWithName:(NSString *)bundleName {
    ...
    NSBundle *bundle = [NSBundle bundleForClass:[GTCPageControl class]];
