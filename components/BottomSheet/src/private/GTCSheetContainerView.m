//
//  GTCSheetContainerView.m
//  GTCatalog
//
//  Created by liuxc on 2018/9/13.
//

#import "GTCSheetContainerView.h"

#import "GTKeyboardWatcher.h"
#import "GTCDraggableView.h"
#import "GTCSheetBehavior.h"
...
@interface GTCSheetContainerView () <GTCDraggableViewDelegate>

@property(nonatomic) GTCSheetState sheetState;
@property(nonatomic) GTCDraggableView *sheet;
@property(nonatomic) UIView *contentView;

@property(nonatomic) UIDynamicAnimator *animator;
@property(nonatomic) GTCSheetBehavior *sheetBehavior;
...
@implementation GTCSheetContainerView


+ (void)initialize {
    if (self != [GTCSheetContainerView class]) {
        return;
    }
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        _sheetState = GTCSheetStatePreferred;

        // Don't set the frame yet because we're going to change the anchor point.
        _sheet = [[GTCDraggableView alloc] initWithFrame:CGRectZero scrollView:scrollView];
        ...
        NSArray *notificationNames = @[
                                       GTCKeyboardWatcherKeyboardWillShowNotification,
                                       GTCKeyboardWatcherKeyboardWillHideNotification,
                                       GTCKeyboardWatcherKeyboardWillChangeFrameNotification
                                       ];
...
- (void)didMoveToWindow {
    ...
    if (self.window) {
        if (!self.sheetBehavior) {
            self.sheetBehavior = [[GTCSheetBehavior alloc] initWithItem:self.sheet];
        }
        [self animatePaneWithInitialVelocity:CGPointZero];
    } else {
...
- (void)updateSheetState {
    ...
    self.sheetState = (currentSheetHeight >= [self maximumSheetHeight] ?
                       GTCSheetStateExtended : GTCSheetStatePreferred);
}
...
- (void)animatePaneWithInitialVelocity:(CGPoint)initialVelocity {
    ...
    __weak GTCSheetContainerView *weakSelf = self;
...
- (void)sheetBehaviorDidUpdate {
    ...
    if (self.sheetState == GTCSheetStateClosed &&
        ...
        self.sheetState = GTCSheetStatePreferred;
    }
}
...
#pragma mark - GTCDraggableViewDelegate

- (CGFloat)maximumHeightForDraggableView:(__unused GTCDraggableView *)view {
    return [self maximumSheetHeight];
}

- (BOOL)draggableView:(__unused GTCDraggableView *)view
...
- (void)draggableView:(__unused GTCDraggableView *)view
draggingEndedWithVelocity:(CGPoint)velocity {
    GTCSheetState targetState;
    if (self.preferredSheetHeight == [self maximumSheetHeight]) {
        // Cannot be extended, only closed.
        targetState = (velocity.y >= 0 ? GTCSheetStateClosed : GTCSheetStatePreferred);
    } else {
        CGFloat currentSheetHeight = CGRectGetMaxY(self.bounds) - CGRectGetMinY(self.sheet.frame);
        if (currentSheetHeight >= self.preferredSheetHeight) {
            targetState = (velocity.y >= 0 ? GTCSheetStatePreferred : GTCSheetStateExtended);
        } else {
            targetState = (velocity.y >= 0 ? GTCSheetStateClosed : GTCSheetStatePreferred);
        }
    }
...
- (void)draggableViewBeganDragging:(__unused GTCDraggableView *)view {
    [self.animator removeAllBehaviors];
    self.isDragging = YES;
}

- (void)setSheetState:(GTCSheetState)sheetState {
