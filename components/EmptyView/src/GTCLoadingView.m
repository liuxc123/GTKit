...
@interface GTCLoadingView ()

@property (nonatomic, strong) GTCLoadingAnimationView *loadingView;
...
- (void)setupSubviews {
    ...
    if (self.loadingAnimationType > 0) {
        [self setupLoadingView];
    } else {
        if (_loadingView) {
            [_loadingView removeSubLayer];
    ...
    //loading 底部控件
    if (_loadingView != nil || _imageView != nil) {
...
- (void)setupLoadingView {
    [self.loadingView showAnimationAtView:self.indicatorView animationType:self.loadingAnimationType];
    self.loadingView.frame = CGRectMake(0, 0, self.loadingSize.width, self.loadingSize.height);
}

- (void)setupIndicatorView {

    if (_loadingView.superview != nil) {
        self.indicatorView.size = CGSizeMake(self.loadingView.width, self.loadingView.height);
    }
    ...
    contentHeight = self.indicatorView.maxY;
}
...
- (void)setupTitleLabel:(NSString *)titleStr{
    ...
    contentHeight = self.titleLabel.maxY;
}


#pragma mark - ------------------ 懒加载 ------------------
- (GTCLoadingAnimationView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[GTCLoadingAnimationView alloc] init];
...
- (void)setLoadingSize:(CGSize)loadingSize {
    if (_loadingSize.width != loadingSize.width || _loadingSize.height != loadingSize.height) {
...
- (void)setLoadingColor:(UIColor *)loadingColor {
    if (_loadingColor != loadingColor) {
        ...
        if (_loadingView) {
            _loadingView.foregroundColor = loadingColor;
        }
    }
}
