...
/**  事件回调 */
typedef void (^GTCActionTapBlock)(void);

/** 内容物背景视图 */
@interface GTCEmptyBaseView : UIView
...
@property (nonatomic, copy) GTCActionTapBlock tapContentViewBlock;

/////////属性传递 (这些属性只用来传递，修改无效)
@property (nonatomic, weak, readonly)   id actionBtnTarget;
@property (nonatomic,assign,readonly)   SEL actionBtnAction;
@property (nonatomic, copy, readonly)   GTCActionTapBlock btnClickBlock;
...
+ (instancetype)emptyActionViewWithImageStr:(NSString *)imageStr
                                  ...
                                  detailStr:(NSString *)detailStr
                                btnTitleStr:(NSString *)btnTitleStr
                              btnClickBlock:(GTCActionTapBlock)btnClickBlock;
