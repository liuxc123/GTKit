...
- (void)setupPromptImageView:(UIImage *)img{
    ...
    contentWidth = self.promptImageView.size.width;
...
- (void)setupDetailLabel:(NSString *)detailStr{
    ...
    contentWidth = width > contentWidth ? width : contentWidth;
    contentHeight = self.detailLabel.maxY;
}

- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(GTCActionTapBlock)btnClickBlock{
