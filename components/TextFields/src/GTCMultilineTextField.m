...
@interface GTCMultilineTextField () {
    UIColor *_cursorColor;

    GTCIntrinsicHeightTextView *_textView;
}
...
@implementation GTCMultilineTextField
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _textView = [[GTCIntrinsicHeightTextView alloc] initWithFrame:CGRectZero];
        [self setupTextView];

        _fundament = [[GTCTextInputCommonFundament alloc] initWithTextInput:self];

        [self commonGTCMultilineTextFieldInitialization];
    }
...
- (instancetype)copyWithZone:(__unused NSZone *)zone {
    GTCMultilineTextField *copy = [[[self class] alloc] initWithFrame:self.frame];
...
- (void)commonGTCMultilineTextFieldInitialization {
    self.backgroundColor = [UIColor clearColor];

    self.textColor = _fundament.textColor;
    // TODO: (#4331) This needs to be converted to the new text scheme.
    self.font = [UIFont gtc_standardFontForMaterialTextStyle:GTCFontTextStyleBody1];
...
- (void)updateConstraints {
    ...
    if (!self.textViewBottomSuperviewBottom) {
        ...
        [NSLayoutConstraint constraintWithItem:self.textView
                                     ...
                                     attribute:NSLayoutAttributeBottom
                                    multiplier:1
                                      constant:-1 * GTCTextInputHalfPadding];
...
- (CGFloat)estimatedTextViewLineHeight {
    ...
    return GTCCeil(self.textView.font.lineHeight * scale) / scale;
}
...
- (void)updateTrailingViewLayout {
    ...
    self.textViewTrailingTrailingViewLeading.active = !GTCCGFloatEqual([self trailingViewAlpha], 0.f);
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:GTCTextFieldTextDidSetTextNotification
                                                        object:self];
}
...
- (GTCIntrinsicHeightTextView *)textView {
    if (!_textView) {
        _textView = [[GTCIntrinsicHeightTextView alloc] initWithFrame:CGRectZero];
        [self setupTextView];
        [self setupUnderlineConstraints];
...
- (void)setTextView:(GTCIntrinsicHeightTextView *)textView {
    ...
    if (currentSize.height != requiredSize.height && self.textView.delegate &&
        [self.layoutDelegate
         respondsToSelector:@selector(multilineTextField:didChangeContentSize:)]) {
            id<GTCMultilineTextInputLayoutDelegate> delegate =
            (id<GTCMultilineTextInputLayoutDelegate>)self.layoutDelegate;
