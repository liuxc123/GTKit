...
static inline NSString *_Nullable GTCNSStringFromCGLineCap(CGLineCap lineCap) {
...
static inline NSString *_Nullable GTCNSStringFromCGLineJoin(CGLineJoin lineJoin) {
...
@interface GTCTextInputBorderView ()

@property(nonatomic, strong, readonly) CAShapeLayer *borderLayer;

@end

@implementation GTCTextInputBorderView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self commonGTCTextInputBorderViewInit];
    }
    return self;
}
...
- (nonnull id)copyWithZone:(nullable __unused NSZone *)zone {
    GTCTextInputBorderView *copy = [[[self class] alloc] initWithFrame:self.frame];
...
- (void)commonGTCTextInputBorderViewInit {
...
- (void)updateBorder {
    ...
    self.borderLayer.lineCap = GTCNSStringFromCGLineCap(self.borderPath.lineCapStyle);
    self.borderLayer.lineJoin = GTCNSStringFromCGLineJoin(self.borderPath.lineJoinStyle);
