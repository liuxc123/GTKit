...
class ShapesShadows: UIView {
  ...
  override class var layerClass: AnyClass {
    get {
      return GTCShapedShadowLayer.self
    }
  }

  func commonInit() {
    guard let layer = layer as? GTCShapedShadowLayer else {
      return
    }
    let shapeGenerator = GTCRectangleShapeGenerator()
    shapeGenerator.topLeftCorner = GTCCutCornerTreatment(cut: 20)
    shapeGenerator.setCorners(GTCCurvedCornerTreatment(size: CGSize(width: 20, height: 20)))
...
class ShapesShadowsController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    UIView.animate(withDuration: 8.0, delay: 0, options: [.autoreverse, .repeat, .allowUserInteraction], animations:{
