...
@interface GTCPendingAnimation : NSObject <CAAction>
...
@implementation GTCShadowMetrics

+ (GTCShadowMetrics *)metricsWithElevation:(CGFloat)elevation {
    if (0.0 < elevation) {
        return [[GTCShadowMetrics alloc] initWithElevation:elevation];
    } else {
        return [GTCShadowMetrics emptyShadowMetrics];
    }
}

- (GTCShadowMetrics *)initWithElevation:(CGFloat)elevation {
    self = [super init];
    if (self) {
        _topShadowRadius = [GTCShadowMetrics ambientShadowBlur:elevation];
        _topShadowOffset = CGSizeMake(0.0, 0.0);
        _topShadowOpacity = kAmbientShadowOpacity;
        _bottomShadowRadius = [GTCShadowMetrics keyShadowBlur:elevation];
        _bottomShadowOffset = CGSizeMake(0.0, [GTCShadowMetrics keyShadowYOff:elevation]);
        _bottomShadowOpacity = kKeyShadowOpacity;
    }
    return self;
}

+ (GTCShadowMetrics *)emptyShadowMetrics {
    static GTCShadowMetrics *emptyShadowMetrics;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        emptyShadowMetrics = [[GTCShadowMetrics alloc] init];
...
- (instancetype)initWithLayer:(id)layer {
    if (self = [super initWithLayer:layer]) {
        if ([layer isKindOfClass:[GTCShadowLayer class]]) {
            GTCShadowLayer *otherLayer = (GTCShadowLayer *)layer;
...
/**
 commonGTCShadowLayerInit creates additional layers based on the values of _elevation and
 _shadowMaskEnabled.
 */
- (void)commonGTCShadowLayerInit {
    if (!_bottomShadow) {
    ...
    GTCShadowMetrics *shadowMetrics = [GTCShadowMetrics metricsWithElevation:_elevation];
...
+ (CGSize)shadowSpreadForElevation:(CGFloat)elevation {
    GTCShadowMetrics *metrics = [GTCShadowMetrics metricsWithElevation:elevation];
...
- (void)configureShadowLayerMaskForLayer:(CAShapeLayer *)maskLayer {
    CGSize shadowSpread = [GTCShadowLayer shadowSpreadForElevation:kShadowElevationDialog];
...
- (void)setElevation:(CGFloat)elevation {
    ...
    GTCShadowMetrics *shadowMetrics = [GTCShadowMetrics metricsWithElevation:elevation];
...
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)event {
    if ([event isEqualToString:@"path"] || [event isEqualToString:@"shadowPath"]) {
        ...
        GTCPendingAnimation *pendingAnim = [[GTCPendingAnimation alloc] init];
...
@implementation GTCPendingAnimation
