...
#import "private/GTCStatusBarShifter.h"
...
// KVO contexts
static char *const kKVOContextGTCFlexibleHeaderView =
"kKVOContextGTCFlexibleHeaderView";

static inline GTCFlexibleHeaderShiftBehavior ShiftBehaviorForCurrentAppContext(
                                                                               GTCFlexibleHeaderShiftBehavior intendedShiftBehavior) {
    if ([[[NSBundle mainBundle] bundlePath] hasSuffix:@".appex"] &&
        intendedShiftBehavior == GTCFlexibleHeaderShiftBehaviorEnabledWithStatusBar) {
        return GTCFlexibleHeaderShiftBehaviorEnabled;
    }
    return intendedShiftBehavior;
}

@interface GTCFlexibleHeaderView () <GTCStatusBarShifterDelegate>
...
@interface GTCFlexibleHeaderScrollViewInfo : NSObject
...
@implementation GTCFlexibleHeaderView {
    ...
    NSMapTable *_trackedScrollViews;  // {UIScrollView:GTCFlexibleHeaderScrollViewInfo}
    GTCFlexibleHeaderScrollViewInfo *_trackingInfo;
    ...
    GTCStatusBarShifter *_statusBarShifter;
    ...
    GTCFlexibleHeaderShadowIntensityChangeBlock _shadowIntensityChangeBlock;
...
// GTCFlexibleHeaderConfiguration properties
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self commonGTCFlexibleHeaderViewInit];
    }
    return self;
}
...
- (void)commonGTCFlexibleHeaderViewInit {
    _wkWebViewClass = NSClassFromString(@"WKWebView");

    _statusBarShifter = [[GTCStatusBarShifter alloc] init];
    ...
    _headerContentImportance = GTCFlexibleHeaderContentImportanceDefault;
    _statusBarHintCanOverlapHeader = YES;

    _minMaxHeightIncludesSafeArea = YES;
    _minimumHeight = kFlexibleHeaderDefaultHeight + [self fhv_topSafeAreaInset];
    ...
    if (![GTCFlexibleHeaderView appearance].backgroundColor) {
...
- (void)fhv_setShadowLayer:(CALayer *)shadowLayer
   intensityDidChangeBlock:(GTCFlexibleHeaderShadowIntensityChangeBlock)block {
...
- (void)fhv_topSafeAreaInsetDidChange {
    ...
    _topSafeAreaGuide.frame = CGRectMake(0, 0, self.bounds.size.width, [self fhv_topSafeAreaInset]);
...
- (CGFloat)fhv_topSafeAreaInset {
    if (self.inferTopSafeAreaInsetFromViewController) {
        ...
        if (topSafeAreaInsetLikelyAffectedByStatusBarVisibility
            ...
            && _shiftBehavior == GTCFlexibleHeaderShiftBehaviorEnabledWithStatusBar
        ...
        return GTCDeviceTopSafeAreaInset();
    }
}

- (void)setTopSafeAreaInset:(CGFloat)topSafeAreaInset {
    _topSafeAreaInset = topSafeAreaInset;

    if (_topSafeAreaInset > 0) {
...
- (void)fhv_removeInsetsFromScrollView:(UIScrollView *)scrollView {
    ...
    GTCFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        return;
    }
...
//
//     _maximumHeight (with safe area insets removed) + [self fhv_topSafeAreaInset]
//
...
- (CGFloat)fhv_enforceInsetsForScrollView:(UIScrollView *)scrollView {
    ...
    GTCFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        info = [[GTCFlexibleHeaderScrollViewInfo alloc] init];
    ...
    if (_minMaxHeightIncludesSafeArea) {
        minimumTopInset = _maximumHeight - [self fhv_topSafeAreaInset];
    } else {
    ...
    BOOL statusBarIsHidden = [UIApplication gtc_safeSharedApplication].statusBarHidden ? YES : NO;
...
- (CGFloat)fhv_accumulatorMax {
    ...
    CGFloat statusBarHeight = [UIApplication gtc_safeSharedApplication].statusBarFrame.size.height;
...
- (BOOL)fhv_canShiftOffscreen {
    return ((_shiftBehavior == GTCFlexibleHeaderShiftBehaviorEnabled ||
             _shiftBehavior == GTCFlexibleHeaderShiftBehaviorEnabledWithStatusBar) &&
...
- (void)fhv_recalculatePhase {
    ...
    if (frame.origin.y < 0) {
        _scrollPhase = GTCFlexibleHeaderScrollPhaseShifting;
    ...
    if (frame.size.height < self.computedMaximumHeight) {
        _scrollPhase = GTCFlexibleHeaderScrollPhaseCollapsing;
    ...
    _scrollPhase = GTCFlexibleHeaderScrollPhaseOverExtending;
...
- (void)fhv_updateLayout {
    ...
    // GTCDeviceTopSafeAreaInset returns an incorrect "best guess" value and we end up storing an
...
- (CGFloat)fhv_anchorLength {
    switch (_headerContentImportance) {
        case GTCFlexibleHeaderContentImportanceDefault:
            return kMaxAnchorLengthFullSwipe;

        case GTCFlexibleHeaderContentImportanceHigh:
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kKVOContextGTCFlexibleHeaderView) {
...
- (void)fhv_startObservingContentOffset {
    [self.trackingScrollView addObserver:self
                              forKeyPath:NSStringFromSelector(@selector(contentOffset))
                                 ...
                                 context:kKVOContextGTCFlexibleHeaderView];
}
...
- (void)setObservesTrackingScrollViewScrollEvents:(BOOL)observesTrackingScrollViewScrollEvents {
    NSAssert(self.shiftBehavior == GTCFlexibleHeaderShiftBehaviorDisabled
...
#pragma mark - GTCStatusBarShifterDelegate

- (void)statusBarShifterNeedsStatusBarAppearanceUpdate:
(__unused GTCStatusBarShifter *)statusBarShifter {
...
- (void)statusBarShifter:(__unused GTCStatusBarShifter *)statusBarShifter
...
- (BOOL)hidesStatusBarWhenCollapsed {
    return (_shiftBehavior == GTCFlexibleHeaderShiftBehaviorEnabledWithStatusBar &&
            !_trackingScrollView.pagingEnabled);
}
...
- (void)setShiftBehavior:(GTCFlexibleHeaderShiftBehavior)shiftBehavior {
    NSAssert((self.observesTrackingScrollViewScrollEvents
              && shiftBehavior == GTCFlexibleHeaderShiftBehaviorDisabled)
    ...
    BOOL needsShiftOnScreen = (_shiftBehavior != GTCFlexibleHeaderShiftBehaviorDisabled &&
                               shiftBehavior == GTCFlexibleHeaderShiftBehaviorDisabled);
...
- (void)setBehavior:(GTCFlexibleHeaderShiftBehavior)behavior {
    self.shiftBehavior = behavior;
}

- (GTCFlexibleHeaderShiftBehavior)behavior {
    return self.shiftBehavior;
}

- (void)changeContentInsets:(GTCFlexibleHeaderChangeContentInsetsBlock)block {
    if (!block) {
        return;
    }
...
- (CGFloat)computedMinimumHeight {
    if (_minMaxHeightIncludesSafeArea) {
        ...
        return _minimumHeight + [self fhv_topSafeAreaInset];
    }
}
...
- (CGFloat)computedMaximumHeight {
    if (_minMaxHeightIncludesSafeArea) {
        ...
        return _maximumHeight + [self fhv_topSafeAreaInset];
    }
}
...
- (void)setMinMaxHeightIncludesSafeArea:(BOOL)minMaxHeightIncludesSafeArea {
    ...
    if (!_hasExplicitlySetMaxHeight) {
        if (_minMaxHeightIncludesSafeArea) {
            _maximumHeight = kFlexibleHeaderDefaultHeight + [self fhv_topSafeAreaInset];
        } else {
...
@implementation GTCFlexibleHeaderScrollViewInfo
@end
