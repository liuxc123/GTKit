//
//  GTCStatusBarShifter.m
//  GTCatalog
//
//  Created by liuxc on 2018/8/22.
//

#import "GTCStatusBarShifter.h"
...
typedef NS_ENUM(NSInteger, GTCStatusBarShifterState) {
    GTCStatusBarShifterStateRealStatusBar,
    GTCStatusBarShifterStateIsSnapshot,
    GTCStatusBarShifterStateInvalidSnapshot,
};

@implementation GTCStatusBarShifter{
    ...
    GTCStatusBarShifterState _snapshotState;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _originalStatusBarHeight = [UIApplication gtc_safeSharedApplication].statusBarFrame.size.height;
...
- (void)invalidateSnapshot {
    [UIView animateWithDuration:kStatusBarBecomesInvalidAnimationDuration
                     animations:^{
                         [self attemptSnapshotState:GTCStatusBarShifterStateInvalidSnapshot];
                     }];
}
...
- (void)attemptSnapshotState:(GTCStatusBarShifterState)snapshotState {
    if (_snapshotState == snapshotState) {
        ...
        if (_snapshotState == GTCStatusBarShifterStateIsSnapshot && [self shouldInvalidateSnapshot]) {
    ...
    if (_snapshotState == GTCStatusBarShifterStateInvalidSnapshot &&
        snapshotState == GTCStatusBarShifterStateIsSnapshot) {
        return;
    }

    // Can't go from real => invalid
    if (_snapshotState == GTCStatusBarShifterStateRealStatusBar &&
        snapshotState == GTCStatusBarShifterStateInvalidSnapshot) {
        return;
    }

    // While disabled, can't leave the real status bar state.
    if (!_enabled && _snapshotState == GTCStatusBarShifterStateRealStatusBar) {
        return;
    }

    // If snapshotting is disabled, then can't go from real => snapshot, but must jump to invalid
    // state.
    if (!_snapshottingEnabled && _snapshotState == GTCStatusBarShifterStateRealStatusBar &&
        snapshotState == GTCStatusBarShifterStateIsSnapshot) {
        snapshotState = GTCStatusBarShifterStateInvalidSnapshot;
    }
    ...
    if (_isChangingInterfaceOrientation && snapshotState == GTCStatusBarShifterStateIsSnapshot) {
        snapshotState = GTCStatusBarShifterStateInvalidSnapshot;
    }
    ...
    switch (_snapshotState) {
        case GTCStatusBarShifterStateRealStatusBar: {
        ...
        case GTCStatusBarShifterStateInvalidSnapshot: {
        ...
        case GTCStatusBarShifterStateIsSnapshot: {
...
- (void)setOffset:(CGFloat)offset {
    ...
    if (statusOffsetY > 0) {
        ...
        if (_snapshotState == GTCStatusBarShifterStateInvalidSnapshot) {
            ...
            [self attemptSnapshotState:GTCStatusBarShifterStateIsSnapshot];
        }
    } else {
        [self attemptSnapshotState:GTCStatusBarShifterStateRealStatusBar];
    }
}
...
- (BOOL)canUpdateStatusBarFrame {
    CGRect statusBarFrame = [[UIApplication gtc_safeSharedApplication] statusBarFrame];
    CGFloat statusBarHeight = MIN(statusBarFrame.size.width, statusBarFrame.size.height);
    return ((statusBarHeight == _originalStatusBarHeight) || _statusBarReplicaView ||
            _snapshotState == GTCStatusBarShifterStateInvalidSnapshot);
}
