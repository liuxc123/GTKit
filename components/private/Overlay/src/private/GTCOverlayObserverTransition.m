//
//  GTCOverlayObserverTransition.m
//  GTCatalog
//
//  Created by liuxc on 2018/8/17.
//

#import "GTCOverlayObserverTransition.h"

#import "GTCOverlayObserverOverlay.h"
#import "GTCOverlayUtilities.h"

#import <QuartzCore/QuartzCore.h>

@interface GTCOverlayObserverTransition ()
...
@implementation GTCOverlayObserverTransition
...
- (void)animateAlongsideTransitionWithOptions:(UIViewAnimationOptions)options
    ...
    if (animations != nil) {
        ...
        __weak GTCOverlayObserverTransition *weakSelf = self;
        void (^animationToRun)(void) = ^{
            GTCOverlayObserverTransition *strongSelf = weakSelf;
...
- (CGRect)compositeFrame {
    ...
    for (GTCOverlayObserverOverlay *overlay in self.overlays) {
...
- (CGRect)compositeFrameInView:(UIView *)targetView {
    return GTCOverlayConvertRectToView(self.compositeFrame, targetView);
}

- (void)enumerateOverlays:(void (^)(id<GTCOverlay> overlay, NSUInteger idx, BOOL *stop))handler {
    if (handler == nil) {
        return;
    }
