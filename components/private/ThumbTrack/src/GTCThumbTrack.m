...
#import "private/GTCThumbTrack+Private.h"
...
@implementation GTCDiscreteDotView
...
static inline CGFloat DistanceFromPointToPoint(CGPoint point1, CGPoint point2) {
    return GTCHypot(point1.x - point2.x, point1.y - point2.y);
}

#if defined(__IPHONE_10_0) && (__IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_10_0)
@interface GTCThumbTrack () <CAAnimationDelegate>
@end
#endif

@interface GTCThumbTrack () <GTCInkTouchControllerDelegate>
@end

@implementation GTCThumbTrack {
    CGFloat _lastDispatchedValue;
    UIColor *_clearColor;
    GTCInkTouchController *_touchController;
    UIView *_trackView;
    CAShapeLayer *_trackMaskLayer;
    CALayer *_trackOnLayer;
    GTCDiscreteDotView *_discreteDots;
    BOOL _shouldDisplayInk;
    GTCNumericValueLabel *_valueLabel;
...
- (instancetype)initWithFrame:(CGRect)frame onTintColor:(UIColor *)onTintColor {
    ...
    if (self) {
        ...
        _thumbView = [[GTCThumbView alloc] initWithFrame:thumbFrame];
        ...
        // Set up ink layer.
        _touchController = [[GTCInkTouchController alloc] initWithView:_thumbView];
        _touchController.delegate = self;

        [_touchController addInkView];

        _touchController.defaultInkView.inkStyle = GTCInkStyleUnbounded;
...
- (void)setShouldDisplayDiscreteDots:(BOOL)shouldDisplayDiscreteDots {
    if (_shouldDisplayDiscreteDots != shouldDisplayDiscreteDots) {
        if (shouldDisplayDiscreteDots) {
            _discreteDots = [[GTCDiscreteDotView alloc] init];
...
- (void)setShouldDisplayDiscreteValueLabel:(BOOL)shouldDisplayDiscreteValueLabel {
    ...
    if (shouldDisplayDiscreteValueLabel) {
        _valueLabel = [[GTCNumericValueLabel alloc]
...
- (BOOL)inkTouchController:(nonnull __unused GTCInkTouchController *)inkTouchController
...
- (void)updateThumbTrackAnimated:(BOOL)animated
    ...
    BOOL activeSegmentShrinking = GTCFabs(self.value - self.filledTrackAnchorValue) <
    GTCFabs(previousValue - self.filledTrackAnchorValue);
    ...
    if (animated) {
        ...
        if (crossesAnchor) {
            ...
            CGFloat animationDurationToAnchor = (GTCFabs(previousValue - _filledTrackAnchorValue) /
                                                 GTCFabs(previousValue - currentValue)) *
            kAnimationDuration;
...
- (void)updateDotsViewActiveSegment {
    if (!GTCCGFloatEqual(self.maximumValue, self.minimumValue)) {
        ...
        CGFloat activeSegmentWidth = GTCFabs(relativeAnchorPoint - relativeValuePoint);
...
- (void)updateViewsMainIsAnimated:(__unused BOOL)animated
    ...
    if (_shouldDisplayDiscreteValueLabel && _numDiscreteValues > 1) {
        ...
        if ([_delegate respondsToSelector:@selector(thumbTrack:stringForValue:)]) {
            ...
            if (CGRectGetWidth(_valueLabel.frame) > 1) {
                ...
                _valueLabel.frame = GTCRectAlignToScale(valueLabelFrame, [UIScreen mainScreen].scale);
            }
        }
    }
    ...
    if (self.enabled) {
        ...
        _trackOnLayer.frame =
        CGRectMake(trackOnXValue, 0, GTCFabs(currentXValue - anchorXValue), _trackHeight);
...
- (CGFloat)relativeValueForValue:(CGFloat)value {
    ...
    if (GTCCGFloatEqual(_minimumValue, _maximumValue)) {
        return _minimumValue;
    }
    CGFloat relValue = (value - _minimumValue) / GTCFabs(_minimumValue - _maximumValue);
    // For RTL we invert the value
    if (self.gtf_effectiveUserInterfaceLayoutDirection == UIUserInterfaceLayoutDirectionRightToLeft) {
        relValue = 1 - relValue;
    }
...
- (CGFloat)closestValueToTargetValue:(CGFloat)targetValue {
    ...
    GTCRound((_numDiscreteValues - 1) * scaledTargetValue) / (_numDiscreteValues - 1.0f);
...
- (void)setValueFromThumbPosition:(CGPoint)position isTap:(BOOL)isTap {
    ...
    if (isTap && _numDiscreteValues == 2) {
        ...
        value = _value < GTCCGFloatEqual(_value, _minimumValue) ? _maximumValue : _minimumValue;
    } else {
        value = [self valueForThumbPosition:position];
    }
    __weak GTCThumbTrack *weakSelf = self;
...
animateThumbAfterMove:YES
     userGenerated:YES
        completion:^{
            GTCThumbTrack *strongSelf = weakSelf;
...
@implementation GTCThumbTrack (Private)

- (GTCNumericValueLabel *)numericValueLabel {
    return _valueLabel;
}

- (GTCInkTouchController *)touchController {
    return _touchController;
}

- (GTCDiscreteDotView *)discreteDotView {
