...
@implementation GTCPathSpace
-(GTCPathSpace *)initWithSpaceType:(GTCPathSpaceType)type value:(CGFloat)value
{
    self = [super init];
    if (self != nil)
    {
...
+(GTCPathSpace *)flexed
{
    return [[self alloc] initWithSpaceType:GTCPathSpaceFlexed value:0];
}

//固定距离，len为长度，每个子视图之间的距离都是len
+(GTCPathSpace *)fixed:(CGFloat)len
{
    return [[self alloc] initWithSpaceType:GTCPathSpaceFixed value:len];
}

//数量距离，根据布局视图的尺寸和指定的数量动态决定。
+(GTCPathSpace *)count:(NSInteger)count
{
    return [[self alloc] initWithSpaceType:GTCPathSpaceCount value:count];
}
...
@implementation GTCCoordinateSetting
{
    __weak GTCPathLayout *_pathLayout;
}

-(GTCCoordinateSetting *)initWithPathLayout:(GTCPathLayout*)pathLayout
{
    self = [super init];
    if (self != nil)
    {
...
{
    if (!_gtcCGPointEqual(_origin, origin))
    {
        _origin = origin;
        [_pathLayout setNeedsLayout];
    }
...
@implementation GTCPathLayout
{
    BOOL _hasOriginView;
    GTCCoordinateSetting  *_coordinateSetting;
    GTCPathSpace *_spaceType;
}
...
-(GTCCoordinateSetting*)coordinateSetting
{
    if (_coordinateSetting == nil)
    {
        _coordinateSetting = [[GTCCoordinateSetting alloc] initWithPathLayout:self];
    }

    return _coordinateSetting;
}

-(GTCPathSpace*)spaceType
{
    if (_spaceType == nil)
    {
        _spaceType = [GTCPathSpace flexed];
    }

    return _spaceType;
}

-(void)setSpaceType:(GTCPathSpace *)spaceType
{
...
-(void)beginSubviewPathPoint:(BOOL)full
{
    ...
    if (full)
    {
        ...
        self.pathPoints = [self gtcCalcPoints:self.pathSubviews path:nil pointIndexArray:pointIndexs lsc:self.gtcCurrentSizeClass];
        self.pointIndexs = pointIndexs;
    }
    else
    {
        self.pathPoints = [self gtcCalcPoints:self.pathSubviews path:nil pointIndexArray:nil lsc:self.gtcCurrentSizeClass];
...
-(CGPathRef)createPath:(NSInteger)subviewCount
{
    ...
    if (self.spaceType.type != GTCPathSpaceFixed)
    {
        [self gtcCalcPathPoints:retPath pPathLen:NULL subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:self.gtcCurrentSizeClass];
    }
    else
    {
        if (subviewCount == -1)
        ...
        [self gtcCalcPathPoints:retPath pPathLen:NULL subviewCount:subviewCount pointIndexArray:nil viewSpace:self.spaceType.value lsc:self.gtcCurrentSizeClass];
    }
...
/*
 ...
 GTCPathLayout *lsc = self.gtcCurrentSizeClass;

 NSArray *pts = nil;
 if (sbs.count > 0)
 {
 if (self.spaceType.type != GTCPathSpace_Fixed)
 {
 CGFloat totalLen = 0;
 NSArray *tempArray = [self gtcCalcPathPoints:path pPathLen:&totalLen subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:lsc];
 BOOL bClose = NO;
 if (tempArray.count > 1)
 {
 CGPoint p1 = [tempArray.firstObject CGPointValue];
 CGPoint p2 = [tempArray.lastObject CGPointValue];
 bClose = [self gtcCalcDistance:p1 with:p2] <=1;
 }

 CGFloat viewSpacing = 0;
 NSInteger sbvcount = sbs.count;
 if (self.spaceType.type == GTCPathSpace_Count)
 ...
 pts = [self gtcCalcPathPoints:nil pPathLen:NULL subviewCount:sbs.count pointIndexArray:nil viewSpace:viewSpacing lsc:lsc];

 }
 else
 {
 pts = [self gtcCalcPathPoints:path pPathLen:NULL subviewCount:sbs.count pointIndexArray:nil viewSpace:self.spaceType.value lsc:lsc];
 }
 }
...
-(CGSize)calcLayoutRect:(CGSize)size isEstimate:(BOOL)isEstimate pHasSubLayout:(BOOL*)pHasSubLayout sizeClass:(GTCSizeClass)sizeClass sbs:(NSMutableArray*)sbs
{
    ...
    //算路径子视图的。
    sbs = [self gtcGetLayoutSubviewsFrom:self.pathSubviews];
    ...
    NSArray *pts = [self gtcCalcPoints:sbs path:path pointIndexArray:nil lsc:lsc];
    ...
    for (int i = 0; i < sbs.count; i++)
    {
        ...
        if (sbvsc.widthSizeInner.dimeRelaVal != nil)
        {
            ...
            {
                rect.size.width = [sbvsc.widthSizeInner measureWith:(selfSize.width - lsc.gtcLayoutLeftPadding - lsc.gtcLayoutRightPadding) ];
            }
            else
            {
        ...
        if (sbvsc.heightSizeInner.dimeRelaVal != nil)
        {
            ...
            {
                rect.size.height = [sbvsc.heightSizeInner measureWith:(selfSize.height - lsc.gtcLayoutTopPadding - lsc.gtcLayoutBottomPadding) ];
            }
            else
            {
        ...
        if (_gtcCGFloatLess(CGRectGetMinY(rect), minYPos))
        {
            minYPos = CGRectGetMinY(rect);
        }

        if (_gtcCGFloatGreat(CGRectGetMaxY(rect), maxYPos))
        {
            maxYPos = CGRectGetMaxY(rect);
        }

        if (_gtcCGFloatLess(CGRectGetMinX(rect), minXPos))
        {
            minXPos = CGRectGetMinX(rect);
        }

        if (_gtcCGFloatGreat(CGRectGetMaxX(rect), maxXPos))
        {
    ...
    if (sbv != nil)
    {
        ...
        //位置在原点位置。。
        rect.origin.x = (selfSize.width - lsc.gtcLayoutLeftPadding - lsc.gtcLayoutRightPadding)*self.coordinateSetting.origin.x  - rect.size.width *sbv.layer.anchorPoint.x + sbvsc.leftPosInner.absVal + lsc.gtcLayoutLeftPadding - sbvsc.rightPosInner.absVal;
        rect.origin.y = (selfSize.height - lsc.gtcLayoutTopPadding - lsc.gtcLayoutBottomPadding)*self.coordinateSetting.origin.y - rect.size.height * sbv.layer.anchorPoint.y + sbvsc.topPosInner.absVal + lsc.gtcLayoutTopPadding - sbvsc.bottomPosInner.absVal;
    ...
    {
        maxYPos = lsc.gtcLayoutTopPadding + lsc.gtcLayoutBottomPadding;
    }
    ...
    {
        maxXPos = lsc.gtcLayoutLeftPadding + lsc.gtcLayoutRightPadding;
    }
...
{
    return [GTCPathLayoutViewSizeClass new];
}
...
-(CGFloat)gtcCalcDistance:(CGPoint)pt1 with:(CGPoint)pt2
{
    return sqrt(pow(pt1.x - pt2.x, 2) + pow(pt1.y - pt2.y, 2));
}


-(CGPoint)gtcGetNearestDistancePoint:(CGFloat)startArg lastXY:(CGPoint)lastXY distance:(CGFloat)distance viewSpace:(CGFloat)viewSpace pLastValidArg:(CGFloat*)pLastValidArg func:(CGPoint (^)(CGFloat))func
{
    ...
    while(true)
    {
        ...
        while (arg - startArg < 0.0001)
        {
            ...
            if (!isnan(realXY.x) && !isnan(realXY.y))
            {
                ...
                distance +=  [self gtcCalcDistance:realXY with:lastXY];
                if ( _gtcCGFloatGreatOrEqual(distance, viewSpace))
                {
                    if (_gtcCGFloatLessOrEqual(distance - viewSpace, self.distanceError))
                    {
                        *pLastValidArg = arg;
                        return realXY;
                    }
        ...
        if (_gtcCGFloatLessOrEqual(step, 0.001))
        {
            *pLastValidArg = arg;
            break;
        }
...
-(void)gtcCalcPathPointsHelper:(NSMutableArray*)pathPointArray
            ...
            else
            {
                if (subviewCount >= 0)
                {
                    ...
                    {//如果距离超过间距。则需要缩小自变量步长。以便达到最小的间距误差
                        if (_gtcCGFloatGreatOrEqual(distance - viewSpace, self.distanceError) )
                        {
                            realXY = [self gtcGetNearestDistancePoint:arg lastXY:lastXY distance:oldDistance viewSpace:viewSpace pLastValidArg:&lastValidArg func:func];
                        }
                        else
                        {
                            lastValidArg = arg;
                ...
                if (pPathLen != NULL)
                {
                    *pPathLen +=  [self gtcCalcDistance:realXY with:lastXY];
                }
            }
...
/**
 ...
 @return 返回所有路径点
 */
-(NSArray*)gtcCalcPathPoints:(CGMutablePathRef)showPath
                   pPathLen:(CGFloat*)pPathLen
               subviewCount:(NSInteger)subviewCount
            pointIndexArray:(NSMutableArray*)pointIndexArray
                  viewSpace:(CGFloat)viewSpace
                        lsc:(GTCPathLayout*)lsc
{
    NSMutableArray *pathPointArray = [NSMutableArray new];

    CGFloat selfWidth = CGRectGetWidth(self.bounds) - lsc.gtcLayoutLeftPadding - lsc.gtcLayoutRightPadding;
    CGFloat selfHeight = CGRectGetHeight(self.bounds) - lsc.gtcLayoutTopPadding - lsc.gtcLayoutBottomPadding;
    ...
    if (self.rectangularEquation != nil)
    {
        ...
        [self gtcCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(val))
            {
                if (self.coordinateSetting.isReverse)
                {
                    return CGPointMake(val + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -arg : arg) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);

                }
                else
                {
                    return CGPointMake(arg + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val : val) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);
    ...
    else if (self.parametricEquation != nil)
    {
        ...
        [self gtcCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray  viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(val.x) && !isnan(val.y))
            {
                if (self.coordinateSetting.isReverse)
                {
                    return CGPointMake(val.y + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val.x : val.x) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);

                }
                else
                {
                    return CGPointMake(val.x + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val.y : val.y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);
                }
            }
            else
                return CGPointMake(NAN, NAN);
        ...
        [self gtcCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray  viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(r))
            {
                if (self.coordinateSetting.isReverse)
                {
                    ...
                    return CGPointMake(r * sin(arg / 180.0 * M_PI) + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -y : y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);
                ...
                else
                {
                    ...
                    return CGPointMake(r * cos(arg / 180 * M_PI) + selfWidth * self.coordinateSetting.origin.x + lsc.gtcLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -y : y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtcLayoutTopPadding);
                }
            }
            else
...
-(NSArray*)gtcCalcPoints:(NSArray*)sbs path:(CGMutablePathRef)path pointIndexArray:(NSMutableArray*)pointIndexArray lsc:(GTCPathLayout*)lsc
{
    if (sbs.count > 0)
    {
        if (self.spaceType.type != GTCPathSpaceFixed)
        {
            ...
            NSArray *tempArray = [self gtcCalcPathPoints:path pPathLen:&pathLen subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:lsc];
            ...
            NSInteger sbvcount = sbs.count;
            if (self.spaceType.type == GTCPathSpaceCount)  //如果是固定数量则按固定数量来分配间距
                sbvcount = (NSInteger)self.spaceType.value;
            ...
            //有间距后再重新计算一遍
            return [self gtcCalcPathPoints:nil pPathLen:NULL subviewCount:sbs.count pointIndexArray:pointIndexArray viewSpace:viewSpace lsc:lsc];

        }
        else
        {
            return [self gtcCalcPathPoints:path pPathLen:NULL subviewCount:sbs.count pointIndexArray:pointIndexArray viewSpace:self.spaceType.value lsc:lsc];
        }
    }
