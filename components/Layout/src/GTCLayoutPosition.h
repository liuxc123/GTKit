 ...
 *视图的布局位置类，用于定位视图在布局视图中的位置。位置可分为水平方向的位置和垂直方向的位置，在视图定位时必要同时指定水平方向的位置和垂直方向的位置。水平方向的位置可以分为左，水平居中，右三种位置，垂直方向的位置可以分为上，垂直居中，下三种位置。
 其中的offset方法可以用来设置布局位置的偏移值,一般只在equalTo设置为GTCLayoutPosition或者NSArray时配合使用。比如A.leftPos.equalTo(B.rightPos).offset(5)表示A在B的右边再偏移5个点
 ...
 +-----------+--------+---------------------+-------+------+--------+---------+----------+----------+---------------+------+
 |method\val |NSNumber|NSArray<GTCLayoutPosition*>|leftPos|topPos|rightPos|bottomPos|centerXPos|centerYPos|UILayoutSupport|UIView|
 +-----------+--------+---------------------+-------+------+--------+---------+----------+----------+---------------+------+
 ...
 */
/*
 The GTCLayoutPosition is the layout position object of the UIView. It is used to set the position relationship at the six directions of left, top, right, bottom, horizontal center, vertical center between the view and sibling views or Layoutview.

 you can use the equalTo() method of GTCLayoutPosition to set as below:
 ...
 2.GTCLayoutPosition: the layout position depends on aother layout position. e.g. A.leftPos.equalTo(B.rightPos) indicates that A is on the right side of B.
 3.NSArray<GTCLayoutPosition*>: all views in the array and View are centered. e.g. A.centerXPos.equalTo(@[B.centerXPos, C.centerXPos]) indicates that A,B,C are overall horizontal centered.
 ...
 you can use offset() method of GTCLayoutPosition to set offset of the layout position,but it is generally used together when equalTo() is set at GTCLayoutPosition or NSArray. e.g. A.leftPos.equalTo(B.rightPos).offset(5) indicates that A is on the right side of B and increase 5 point offset.

 you can use max and min method of GTCLayoutPosition to limit the maximum and minimum postion value. e.g. A.leftPos.min (10).Max (40) indicates that the minimum value of the left boundary is 10 and the maximum is 40. The min and max method always be used together when the subview' position of MyLinearLayout or MyFrameLayout is set at the relative position(0,1].
...
/**
 ...
 @endcode
 */
@interface GTCLayoutPosition : NSObject <NSCopying>
...
/**
 ...
 //这里设置布局视图的左边和右边以及顶部的边距都是在父视图的安全区域外再缩进10个点的位置。你会注意到这里面定义了一个特殊的位置GTCLayoutPosition.safeAreaMargin。
 //GTCLayoutPosition.safeAreaMargin表示视图的边距不是一个固定的值而是所在的父视图的安全区域。这样布局视图就不会延伸到安全区域以外去了。
 //GTCLayoutPosition.safeAreaMargin是同时支持iOS11和以下的版本的，对于iOS11以下的版本则顶部安全区域是状态栏以下的位置。
 //因此只要你设置边距为GTCLayoutPosition.safeAreaMargin则可以同时兼容所有iOS的版本。。
 layoutView.leadingPos.equalTo(@(GTCLayoutPosition.safeAreaMargin)).offset(10);
 layoutView.trailingPos.equalTo(@(GTCLayoutPosition.safeAreaMargin)).offset(10);
 layoutView.topPos.equalTo(@(GTCLayoutPosition.safeAreaMargin)).offset(10);

 //如果你的左右边距都是安全区域，那么可以用下面的方法来简化设置。您可以注释掉这句代码看看效果。
 // layoutView.myLeading = layoutView.myTrailing = GTCLayoutPosition.safeAreaMargin;
 @endcode

 @code

 //在一个没有导航条的界面中，因为iPhoneX和其他设备的状态栏的高度不一致。所以你可以让布局视图的topPadding设置如下：
 layoutView.topPadding = GTCLayoutPosition.safeAreaMargin + 10;  //顶部内边距是安全区域外加10。那么这个和设置如下的：
 ...
 @note
 需要注意的是这个值并不是一个真值，只是一个特殊值，不能用于读取。而且只能用于在GTCLayoutPosition的equalTo方法和布局视图上的padding属性上使用，其他地方使用后果未可知。

 */
@property(class, nonatomic, assign,readonly) CGFloat gtc_safeAreaMargin;
...
/**
 ...
 2. GTCLayoutPosition表示位置依赖于另外一个位置。

 3. NSArray<GTCLayoutPosition*>表示位置和数组里面的其他位置整体居中。
 ...
 6. nil表示位置的值被清除。
 */
-(GTCLayoutPosition* (^)(id val))equalTo;

-(GTCLayoutPosition* (^)(id val))gtc_equalTo;
...
/**
 ...
 @endcode
 */
-(GTCLayoutPosition* (^)(CGFloat val))offset;

-(GTCLayoutPosition* (^)(CGFloat val))gtc_offset;


/**
 *设置位置的最小边界数值，min方法是lBound方法的简化版本。比如：A.min(10) <==>  A.lBound(@10, 0)
 */
-(GTCLayoutPosition* (^)(CGFloat val))min;

-(GTCLayoutPosition* (^)(CGFloat val))gtc_min;


/**
 *设置布局位置的最小边界值。 如果位置对象没有设置最小边界值，那么最小边界默认就是无穷小-CGFLOAT_MAX。lBound方法除了能设置为NSNumber外，还可以设置为GTCLayoutPosition值，并且还可以指定最小位置的偏移量值。只有在相对布局中的子视图的位置对象才能设置最小边界值为GTCLayoutPosition类型的值，其他类型布局中的子视图只支持NSNumber类型的最小边界值。

 posVal: 指定位置边界值。可设置的类型有NSNumber和GTCLayoutPosition类型，前者表示最小位置不能小于某个常量值，而后者表示最小位置不能小于另外一个位置对象所表示的位置值。
 ...
 @endcode
 */
-(GTCLayoutPosition* (^)(id posVal, CGFloat offsetVal))lBound;

-(GTCLayoutPosition* (^)(id posVal, CGFloat offsetVal))gtc_lBound;
 ...
 */
-(GTCLayoutPosition* (^)(CGFloat val))max;

-(GTCLayoutPosition* (^)(CGFloat val))gtc_max;


/**
 设置布局位置的最大边界值。 如果位置对象没有设置最大边界值，那么最大边界默认就是无穷大CGFLOAT_MAX。uBound方法除了能设置为NSNumber外，还可以设置为GTCLayoutPosition值，并且还可以指定最大位置的偏移量值。只有在相对布局中的子视图的位置对象才能设置最大边界值为GTCLayoutPosition类型的值，其他类型布局中的子视图只支持NSNumber类型的最大边界值。
 ...
 posVal 指定位置边界值。可设置的类型有NSNumber和GTCLayoutPosition类型，前者表示最大位置不能大于某个常量值，而后者表示最大位置不能大于另外一个位置对象所表示的位置值。

 offsetVal 指定位置边界值的偏移量。
 */
-(GTCLayoutPosition* (^)(id posVal, CGFloat offsetVal))uBound;

-(GTCLayoutPosition* (^)(id posVal, CGFloat offsetVal))gtc_uBound;
