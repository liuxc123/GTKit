...
@implementation GTCLinearLayout
...
- (instancetype)initWithFrame:(CGRect)frame orientation:(GTCOrientation)orientation
{
    ...
    if (self)
    {
        GTCLinearLayout *lsc = self.gtcCurrentSizeClass;
...
+(instancetype)linearLayoutWithOrientation:(GTCOrientation)orientation
{
...
- (GTCSubviewsShrinkType)shrinkType
{
    return self.gtcCurrentSizeClass.shrinkType;

}

- (void)setShrinkType:(GTCSubviewsShrinkType)shrinkType
{
    GTCLinearLayout *lsc = self.gtcCurrentSizeClass;
...
-(void)equalizeSubviews:(BOOL)centered inSizeClass:(GTCSizeClass)sizeClass
{
...
{
    [self equalizeSubviews:centered withSpace:space inSizeClass:GTCSizeClasshAny | GTCSizeClasswAny];
    [self setNeedsLayout];
}

-(void)equalizeSubviews:(BOOL)centered withSpace:(CGFloat)space inSizeClass:(GTCSizeClass)sizeClass
{
    self.gtcFrame.sizeClass = [self fetchLayoutSizeClass:sizeClass];
    for (UIView *sbv in self.subviews)
        sbv.gtcFrame.sizeClass = [sbv fetchLayoutSizeClass:sizeClass];

    if (self.orientation == GTCOrientationVert)
    {
        [self gtcEqualizeSubviewsForVert:centered withSpace:space];
    }
    else
    {
        [self gtcEqualizeSubviewsForHorz:centered withSpace:space];
    }

    self.gtcFrame.sizeClass = self.gtcDefaultSizeClass;
    for (UIView *sbv in self.subviews)
        sbv.gtcFrame.sizeClass = sbv.gtcDefaultSizeClass;

}

-(void)equalizeSubviewsSpace:(BOOL)centered
{
    [self equalizeSubviewsSpace:centered inSizeClass:GTCSizeClasshAny | GTCSizeClasswAny];
    [self setNeedsLayout];

}

-(void)equalizeSubviewsSpace:(BOOL)centered inSizeClass:(GTCSizeClass)sizeClass
{
    ...
    if (self.orientation == GTCOrientationVert)
    {
        [self gtcEqualizeSubviewsSpaceForVert:centered];
    }
    else
    {
        [self gtcEqualizeSubviewsSpaceForHorz:centered];
    }
...
-(CGSize)calcLayoutRect:(CGSize)size isEstimate:(BOOL)isEstimate pHasSubLayout:(BOOL*)pHasSubLayout sizeClass:(GTCSizeClass)sizeClass sbs:(NSMutableArray *)sbs
{
    ...
    GTCGravity horzGravity = lsc.gravity & GTCGravityVertMask;
    GTCOrientation oreintation = lsc.orientation;
    ...
    for (UIView *sbv in sbs)
    {
        ...
        {
            [self gtcAdjustSubviewWrapContent:sbv sbvsc:sbvsc orientation:oreintation gravity:(oreintation == GTCOrientationVert)? horzGravity : vertGravity];
    ...
    if (oreintation == GTCOrientationVert)
    {
        if (vertGravity != GTCGravityNone)
            selfSize = [self gtcLayoutSubviewsForVertGravity:selfSize sbs:sbs lsc:lsc];
        else
            selfSize = [self gtcLayoutSubviewsForVert:selfSize sbs:sbs lsc:lsc];
    }
    else
    {
        if (horzGravity != GTCGravityNone)
            selfSize = [self gtcLayoutSubviewsForHorzGravity:selfSize sbs:sbs lsc:lsc];
        else
            selfSize = [self gtcLayoutSubviewsForHorz:selfSize sbs:sbs lsc:lsc];
    }

    //绘制智能线。
    if (!isEstimate)
    {
        [self gtcSetLayoutIntelligentBorderline:sbs lsc:lsc];
    }
...
{
    return [GTCLinearLayoutViewSizeClass new];
}


#pragma mark -- Private Methods
//调整子视图的wrapContent设置
- (void)gtcAdjustSubviewWrapContent:(UIView*)sbv sbvsc:(UIView*)sbvsc orientation:(GTCOrientation)orientation  gravity:(GTCGravity)gravity
{
    if (orientation == GTCOrientationVert)
    {
...
//设置智能边界线
-(void)gtcSetLayoutIntelligentBorderline:(NSArray*)sbs lsc:(GTCLinearLayout*)lsc
{
    if (self.intelligentBorderline == nil)
        return;

    BOOL isVert = (lsc.orientation == GTCOrientationVert);
    CGFloat subviewSpace = (lsc.orientation == GTCOrientationVert) ? lsc.subviewVSpace : lsc.subviewHSpace;
    for (int i = 0; i < sbs.count; i++)
    {
        ...
        {
            BOOL ok = YES;
            if ([prevSiblingView isKindOfClass:[GTCBaseLayout class]] && subviewSpace == 0)
            {
                GTCBaseLayout *prevSiblingLayout = (GTCBaseLayout*)prevSiblingView;
        ...
        if (nextSiblingView != nil && (![nextSiblingView isKindOfClass:[GTCBaseLayout class]] || subviewSpace != 0))
        {
            if (isVert)
...
//计算得到最大的包裹宽度
- (CGSize)gtcCalcMaxWrapWidth:(NSArray *)sbs selfSize:(CGSize)selfSize paddingHorz:(CGFloat)paddingHorz lsc:(GTCLinearLayout*)lsc
{
...
-(CGSize)gtcLayoutSubviewsForVert:(CGSize)selfSize sbs:(NSArray*)sbs lsc:(GTCLinearLayout*)lsc
{
    ...
    selfSize = [self gtcCalcMaxWrapWidth:sbs selfSize:selfSize paddingHorz:paddingHorz lsc:lsc];   //调整自身的宽度
    ...
    for (UIView *sbv in sbs)
    {
        ...
        [self gtcCalcLeadingTrailingRect:horzGravity
                               selfSize:selfSize
                                 rect_p:&rect
                                    sbv:sbv
        ...
        if (sbvsc.wrapContentHeight && ![sbv isKindOfClass:[GTCBaseLayout class]] && sbvsc.weight == 0)
        {//特殊处理高度等于wrap的情况。
    ...
    if (lsc.wrapContentHeight && totalWeight != 0)
    {
        ...
        selfSize.height = [self gtcValidMeasure:lsc.heightSizeInner sbv:self calcSize:tempSelfHeight sbvSize:selfSize selfLayoutSize:self.superview.bounds.size];
    ...
    GTCSubviewsShrinkType sstMode = lsc.shrinkType & 0x0F; //压缩的模式
    GTCSubviewsShrinkType sstContent = lsc.shrinkType & 0xF0; //压缩内容
    if (_gtcCGFloatLessOrEqual(floatingHeight, 0))
    {
        if (sstMode != GTCSubviewsShrinkNone)
        {
            if (sstContent == GTCSubviewsShrinkSize)
            {//压缩尺寸
                if (fixedSizeSbs.count > 0 && totalWeight != 0 && floatingHeight < 0 && selfSize.height > 0)
                {
                    if (sstMode == GTCSubviewsShrinkAverage)
                    {//均分。
                        CGFloat averageHeight = floatingHeight / fixedSizeSbs.count;

                        for (UIView *fsbv in fixedSizeSbs)
                        {
                            fsbv.gtcFrame.height += averageHeight;
                        }
                    }
                    else if (_gtcCGFloatNotEqual(fixedSizeHeight, 0))
                    {//按比例分配。
                        for (UIView *fsbv in fixedSizeSbs)
                        {
                            fsbv.gtcFrame.height += floatingHeight * (fsbv.gtcFrame.height / fixedSizeHeight);
                        }

                    }
                }
            }
            else if (sstContent == GTCSubviewsShrinkSpace)
            {//压缩间距
                if (fixedSpaceCount > 0 && floatingHeight < 0 && selfSize.height > 0 && fixedSpaceHeight > 0)
                {
                    ...
                    else if (sstMode == GTCSubviewsShrinkWeight)
                    {
    ...
    //如果有浮动尺寸或者有压缩模式
    if (totalWeight != 0 || (sstMode != GTCSubviewsShrinkNone && _gtcCGFloatLessOrEqual(floatingHeight, 0)))
    {
        pos = paddingTop;
        for (UIView *sbv in sbs) {
            ...
            //分别处理相对顶部间距和绝对顶部间距
            if ([self gtcIsRelativePos:topSpace])
            {
                CGFloat topSpaceWeight = topSpace;
                topSpace = _gtcCGFloatRound((topSpaceWeight / totalWeight) * floatingHeight);
                floatingHeight -= topSpace;
                totalWeight -= topSpaceWeight;
                if (_gtcCGFloatLessOrEqual(topSpace, 0))
                    topSpace = 0;
            }
            else
            {
            ...
            pos += [self gtcValidMargin:sbvsc.topPosInner sbv:sbv calcPos:topSpace + sbvsc.topPosInner.offsetVal selfLayoutSize:selfSize];
            rect.origin.y = pos;

            //分别处理相对高度和绝对高度
            if (weight > 0)
            {
                CGFloat h = _gtcCGFloatRound((weight / totalWeight) * floatingHeight);
                floatingHeight -= h;
                totalWeight -= weight;
                if (_gtcCGFloatLessOrEqual(h, 0))
                    h = 0;

                rect.size.height = [self gtcValidMeasure:sbvsc.heightSizeInner sbv:sbv calcSize:h sbvSize:rect.size selfLayoutSize:selfSize];
            }

            pos += rect.size.height;

            //分别处理相对底部间距和绝对底部间距
            if ([self gtcIsRelativePos:bottomSpace])
            {
                CGFloat bottomSpaceWeight = bottomSpace;
                bottomSpace = _gtcCGFloatRound((bottomSpaceWeight / totalWeight) * floatingHeight);
                floatingHeight -= bottomSpace;
                totalWeight -= bottomSpaceWeight;
                if ( _gtcCGFloatLessOrEqual(bottomSpace, 0))
                    bottomSpace = 0;
            ...
            pos += [self gtcValidMargin:sbvsc.bottomPosInner sbv:sbv calcPos:bottomSpace + sbvsc.bottomPosInner.offsetVal selfLayoutSize:selfSize];
...
-(CGSize)gtcLayoutSubviewsForHorz:(CGSize)selfSize sbs:(NSArray*)sbs lsc:(GTCLinearLayout*)lsc
{
    ...
    for (UIView *sbv in sbs)
    {
        ...
        else
        {
            ...
            vWidth = [self gtcValidMeasure:sbvsc.widthSizeInner sbv:sbv calcSize:vWidth sbvSize:sbvFrame.frame.size selfLayoutSize:selfSize];
            sbvFrame.width = vWidth;
            fixedWidth += vWidth;
    ...
    if (lsc.wrapContentWidth && totalWeight != 0)
    {
        ...
        selfSize.width = [self gtcValidMeasure:lsc.widthSizeInner sbv:self calcSize:tempSelfWidth sbvSize:selfSize selfLayoutSize:self.superview.bounds.size];
    ...
    if (_gtcCGFloatLessOrEqual(floatingWidth, 0))
    {
        ...
        if (sstMode == GTCSubviewsShrinkAuto && flexedSizeSbs.count != 2)
            sstMode = GTCSubviewsShrinkNone;
            ...
            if (sstContent == GTCSubviewsShrinkSize)
            {
                if (fixedSizeSbs.count > 0 && totalWeight != 0 && floatingWidth < 0 && selfSize.width > 0)
                {
                    ...
                    if (sstMode == GTCSubviewsShrinkAverage)
                    {
                        ...
                        for (UIView *fsbv in fixedSizeSbs)
                        {
                            fsbv.gtcFrame.width += averageWidth;

                        }
                    }
                    else if (sstMode == GTCSubviewsShrinkAuto)
                    {

                        UIView *leadingView = flexedSizeSbs[0];
                        UIView *trailingView = flexedSizeSbs[1];

                        CGFloat leadingWidth = leadingView.gtcFrame.width;
                        CGFloat trailingWidth = trailingView.gtcFrame.width;
                        ...
                        if (_gtcCGFloatGreat(leadingWidth, halfLayoutWidth) && _gtcCGFloatGreat(trailingWidth,halfLayoutWidth))
                        {
                            leadingView.gtcFrame.width = halfLayoutWidth;
                            trailingView.gtcFrame.width = halfLayoutWidth;
                        }
                        else if ((_gtcCGFloatGreat(leadingWidth, halfLayoutWidth) || _gtcCGFloatGreat(trailingWidth, halfLayoutWidth)) && (_gtcCGFloatGreat(leadingWidth + trailingWidth, layoutWidth)))
                        {

                            if (_gtcCGFloatGreat(leadingWidth, halfLayoutWidth))
                            {
                                trailingView.gtcFrame.width = trailingWidth;
                                leadingView.gtcFrame.width = layoutWidth - trailingWidth;
                            }
                            else
                            {
                                leadingView.gtcFrame.width = leadingWidth;
                                trailingView.gtcFrame.width = layoutWidth - leadingWidth;
                            }
                    ...
                    else if (_gtcCGFloatNotEqual(fixedSizeWidth, 0))
                    {//按比例分配。
                        for (UIView *fsbv in fixedSizeSbs)
                        {
                            fsbv.gtcFrame.width += floatingWidth * (fsbv.gtcFrame.width / fixedSizeWidth);
                        }
    ...
    for (UIView *sbv in sbs) {
        ...
        //计算出先对左边边距和绝对左边边距
        if ([self gtcIsRelativePos:leadingSpace])
        {
            CGFloat leadingSpaceWeight = leadingSpace;
            leadingSpace = _gtcCGFloatRound((leadingSpaceWeight / totalWeight) * floatingWidth);
            floatingWidth -= leadingSpace;
            totalWeight -= leadingSpaceWeight;
            if (_gtcCGFloatLessOrEqual(leadingSpace, 0))
                leadingSpace = 0;
        ...
        pos += [self gtcValidMargin:sbvsc.leadingPosInner sbv:sbv calcPos:leadingSpace + sbvsc.leadingPosInner.offsetVal selfLayoutSize:selfSize];
        ...
        if (weight > 0)
        {
            CGFloat w = _gtcCGFloatRound((weight / totalWeight) * floatingWidth);
            floatingWidth -= w;
            totalWeight -= weight;

            if (_gtcCGFloatLessOrEqual(w, 0))
                w = 0;
        ...
        //计算相对的右边边距和绝对的右边边距
        if ([self gtcIsRelativePos:trailingSpace])
        {
            CGFloat trailingSpaceWeight = trailingSpace;
            trailingSpace = _gtcCGFloatRound((trailingSpaceWeight / totalWeight) * floatingWidth);
            floatingWidth -= trailingSpace;
            totalWeight -= trailingSpaceWeight;
            if (_gtcCGFloatLessOrEqual(trailingSpace, 0))
                trailingSpace = 0;
        }
        else
        {
        ...
        pos += [self gtcValidMargin:sbvsc.trailingPosInner sbv:sbv calcPos:trailingSpace + sbvsc.trailingPosInner.offsetVal selfLayoutSize:selfSize];
        ...
        //如果高度是浮动的则需要调整高度。
        if (sbvsc.wrapContentHeight && ![sbv isKindOfClass:[GTCBaseLayout class]] && sbvsc.heightSizeInner.dimeRelaVal.view != self)
        {
            rect.size.height = [self gtcHeightFromFlexedHeightView:sbv sbvsc:sbvsc inWidth:rect.size.width];
        }
        ...
        if (lsc.wrapContentHeight)
        {
            ...
            {
                maxSubviewHeight = [self gtcCalcSelfSize:maxSubviewHeight subviewSize:rect.size.height headPos:sbvsc.topPosInner centerPos:sbvsc.centerYPosInner tailPos:sbvsc.bottomPosInner];
            }
        }
        else
        {
            [self gtcCalcSubviewTopBottomRect:vertGravity
                                    selfSize:selfSize
                                      rect_p:&rect
                                         sbv:sbv
            ...
            if (vertGravity == GTCGravityVertBaseline && baselinePos == CGFLOAT_MAX && self.baselineBaseView == sbv)
            {
...
-(CGSize)gtcLayoutSubviewsForVertGravity:(CGSize)selfSize sbs:(NSArray*)sbs lsc:(GTCLinearLayout*)lsc
{
    ...
    selfSize = [self gtcCalcMaxWrapWidth:sbs selfSize:selfSize paddingHorz:paddingHorz lsc:lsc];
    ...
    for (UIView *sbv in sbs)
    {
        ...
        totalHeight += [self gtcValidMargin:sbvsc.topPosInner sbv:sbv calcPos:[sbvsc.topPosInner realPosIn:floatingHeight] selfLayoutSize:selfSize];

        totalHeight += rect.size.height;

        totalHeight += [self gtcValidMargin:sbvsc.bottomPosInner sbv:sbv calcPos:[sbvsc.bottomPosInner realPosIn:floatingHeight] selfLayoutSize:selfSize];

        sbvFrame.frame = rect;

        //如果子布局视图是wrap属性则不进行扩展。
        if (vertGravity == GTCGravityVertFill && [sbv isKindOfClass:[GTCBaseLayout class]])
        {
            if (sbvsc.wrapContentHeight)
            {
                canAddToNoWrapSbs = NO;
    ...
    for (UIView *sbv in sbs)
    {
        ...
        pos += [self gtcValidMargin:sbvsc.topPosInner sbv:sbv calcPos:[sbvsc.topPosInner realPosIn:floatingHeight] selfLayoutSize:selfSize];
        ...
        pos += [self gtcValidMargin:sbvsc.bottomPosInner sbv:sbv calcPos:[sbvsc.bottomPosInner realPosIn:floatingHeight] selfLayoutSize:selfSize];
...
-(CGSize)gtcLayoutSubviewsForHorzGravity:(CGSize)selfSize sbs:(NSArray*)sbs lsc:(GTCLinearLayout*)lsc
{
    CGFloat paddingTop = lsc.gtcLayoutTopPadding;
    CGFloat paddingBottom = lsc.gtcLayoutBottomPadding;
    CGFloat paddingLeading = lsc.gtcLayoutLeadingPadding;
    ...
    for (UIView *sbv in sbs)
    {
        ...
        totalWidth += [self gtcValidMargin:sbvsc.leadingPosInner sbv:sbv calcPos:[sbvsc.leadingPosInner realPosIn:floatingWidth] selfLayoutSize:selfSize];

        totalWidth += rect.size.width;


        totalWidth += [self gtcValidMargin:sbvsc.trailingPosInner sbv:sbv calcPos:[sbvsc.trailingPosInner realPosIn:floatingWidth] selfLayoutSize:selfSize];


        sbvFrame.frame = rect;

        //如果子视图是包裹属性则也不加入。
        if (horzGravity == GTCGravityHorzFill && [sbv isKindOfClass:[GTCBaseLayout class]])
        {
            if (sbvsc.wrapContentWidth)
            {
                canAddToNoWrapSbs = NO;
    ...
    for (UIView *sbv in sbs)
    {
        ...
        pos += [self gtcValidMargin:sbvsc.leadingPosInner sbv:sbv calcPos:[sbvsc.leadingPosInner realPosIn:floatingWidth] selfLayoutSize:selfSize];
        ...
        [self gtcCalcSubviewTopBottomRect:vertGravity selfSize:selfSize rect_p:&rect sbv:sbv paddingBottom:paddingBottom paddingTop:paddingTop baselinePos:baselinePos sbvsc:sbvsc lsc:lsc];
        ...
        pos += [self gtcValidMargin:sbvsc.trailingPosInner sbv:sbv calcPos:[sbvsc.trailingPosInner realPosIn:floatingWidth] selfLayoutSize:selfSize];
...
- (void)gtcCalcLeadingTrailingRect:(GTCGravity)horzGravity selfSize:(CGSize)selfSize rect_p:(CGRect *)rect_p sbv:(UIView *)sbv paddingTrailing:(CGFloat)paddingTrailing paddingLeading:(CGFloat)paddingLeading sbvsc:(UIView *)sbvsc lsc:(GTCLinearLayout*)lsc
{
    if (sbvsc.widthSizeInner.dimeNumVal != nil)
    ...
    rect_p->size.width = [self gtcValidMeasure:sbvsc.widthSizeInner sbv:sbv calcSize:rect_p->size.width sbvSize:rect_p->size selfLayoutSize:selfSize];


    [self gtcCalcHorzGravity:[self gtcGetSubviewHorzGravity:sbv sbvsc:sbvsc horzGravity:horzGravity] sbv:sbv sbvsc:sbvsc paddingLeading:paddingLeading paddingTrailing:paddingTrailing selfSize:selfSize pRect:rect_p];
}

- (void)gtcCalcSubviewTopBottomRect:(GTCGravity)vertGravity selfSize:(CGSize)selfSize rect_p:(CGRect *)rect_p sbv:(UIView *)sbv paddingBottom:(CGFloat)paddingBottom paddingTop:(CGFloat)paddingTop baselinePos:(CGFloat)baselinePos sbvsc:(UIView *)sbvsc lsc:(GTCLinearLayout*)lsc
{
    //计算高度
    if (sbvsc.heightSizeInner.dimeRelaVal != nil && sbvsc.heightSizeInner.dimeRelaVal == lsc.heightSizeInner)
    {
    ...
    rect_p->size.height = [self gtcValidMeasure:sbvsc.heightSizeInner sbv:sbv calcSize:rect_p->size.height sbvSize:rect_p->size selfLayoutSize:selfSize];

    [self gtcCalcVertGravity:[self gtcGetSubviewVertGravity:sbv sbvsc:sbvsc vertGravity:vertGravity] sbv:sbv sbvsc:sbvsc paddingTop:paddingTop paddingBottom:paddingBottom  baselinePos:baselinePos selfSize:selfSize pRect:rect_p];
}

-(CGFloat)gtcCalcSelfSize:(CGFloat)selfSize subviewSize:(CGFloat)subviewSize headPos:(GTCLayoutPosition*)headPos centerPos:(GTCLayoutPosition*)centerPos tailPos:(GTCLayoutPosition*)tailPos
{
    CGFloat totalWeight = 0;
    ...
    if (_gtcCGFloatLessOrEqual(1, totalWeight))
        tempSize = 0;
    else
        tempSize /=(1 - totalWeight);


    CGFloat leadingMargin = [self gtcValidMargin:headPos sbv:headPos.view calcPos:[headPos realPosIn:tempSize] selfLayoutSize:CGSizeZero];

    CGFloat centerMargin = [self gtcValidMargin:centerPos sbv:centerPos.view calcPos:[centerPos realPosIn:tempSize] selfLayoutSize:CGSizeZero];

    CGFloat trailingMargin = [self gtcValidMargin:tailPos sbv:tailPos.view calcPos:[tailPos realPosIn:tempSize] selfLayoutSize:CGSizeZero];

    tempSize = subviewSize + leadingMargin + centerMargin + trailingMargin;
    if (_gtcCGFloatGreat(tempSize,selfSize))
    {
        selfSize = tempSize;
    }
...
-(void)gtcEqualizeSubviewsForVert:(BOOL)centered withSpace:(CGFloat)margin
{
...
-(void)gtcEqualizeSubviewsForHorz:(BOOL)centered withSpace:(CGFloat)space
{
...
-(void)gtcEqualizeSubviewsSpaceForVert:(BOOL)centered
{
...
-(void)gtcEqualizeSubviewsSpaceForHorz:(BOOL)centered
{
