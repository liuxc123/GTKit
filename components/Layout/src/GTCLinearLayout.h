...
/**
 ...
 @endcode
 */
@interface GTCLinearLayout : GTCBaseLayout
...
/**
 ...
 1. GTCOrientationVert 表示布局内子视图依次从上到下垂直排列。这个方向是默认方向。

 2. GTCOrientationHorz 表示布局内子视图依次从左到右(RTL从右到左)水平排列。
 */
@property(nonatomic,assign) IBInspectable GTCOrientation orientation;


/**
 用来设置当线性布局中的所有子视图的尺寸和间距的和大于线性布局的尺寸时子视图压缩策略。默认值是GTCSubviewsShrinkNone，表示什么不压缩子视图的尺寸和间距。
 ...
 1. GTCSubviewsShrinkNone | GTCSubviewsShrinkSize(这是布局的默认设置属性):

 这种情况下即使是固定的视图的尺寸超出也不会进行任何压缩！！！！

 2. GTCSubviewsShrinkAverage | GTCSubviewsShrinkSize (平均压缩)
 ...
 3. GTCSubviewsShrinkWeight | GTCSubviewsShrinkSize(按比例压缩)
 ...
 4.GTCSubviewsShrinkAuto (自动压缩，只对水平线性布局有效）
 ...
 @code
 GTCLinearLayout *horzLayout = [GTCLinearLayout linearLayoutWithOrientation:GTCOrientationHorz];
 horzLayout.myHorzMargin = 0;
 horzLayout.wrapContentHeight = YES;
 horzLayout.subviewSpace = 10;  //二者的最小间距不能小于20
 horzLayout.shrinkType = GTCSubviewsShrink_Auto;

 UILabel *A = [UILabel new];
 A.text = @"xxxxxxx";
 A.widthSize.equalTo(A.widthSize); //宽度等于自身内容的宽度，必须要这么设置和 GTCSubviewsShrinkAuto 结合使用。
 ...
 B.text = @"XXXXXXXX";
 B.widthSize.equalTo(B.widthSize); //宽度等于自身内容的宽度，必须要这么设置和 GTCSubviewsShrinkAuto 结合使用。
 B.wrapContentHeight = YES;        //自动换行
 B.leftPos.equalTo(@0.5);         //左边间距是剩余的50%
 [horzLayout addSubview:B];
 @endcode

 5. GTCSubviewsShrinkAverage | GTCSubviewsShrinkSpace (平均压缩间距）
 ...
 6. GTCSubviewsShrinkWeight | GTCSubviewsShrinkSpace: (按比重压缩间距)
...
/**
 ...
 这个属性要和gravity属性设置为GTCGravityVertBaseline配合使用。并且要求这个属性所指定的视图，必须具有font属性。
