...
@implementation GTCBorderlineLayerDelegate
{
...
-(void)updateBorderLayer:(CAShapeLayer**)ppLayer withBorderline:(GTCBorderline*)borderline
{
...
-(void)layoutSublayersOfLayer:(CAShapeLayer *)layer
{
    ...
    if (!_gtcCGRectEqual(layer.frame, layerRect))
    {
...
//布局的事件处理委托对象
@implementation GTCTouchEventDelegate
{
...
BOOL _hasBegin;
__weak GTCBaseLayout * _currentLayout;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_layout != nil && _target != nil && !_forbidTouch && touch.tapCount == 1 && !_hasBegin)
    {
        ...
        [self gtcSetTouchHighlighted];

        _hasDoCancel = NO;
        [self gtcPerformTouch:_touchDownTarget withAction:_touchDownAction];
    }
}
    ...
    if (_layout != nil && _target != nil && _hasBegin && (_layout == _currentLayout || _currentLayout == nil))
    {
        if (_canCallAction)
        {
            ...
            if (fabs(pt.x - _beginPoint.x) > 2 || fabs(pt.y - _beginPoint.y) > 2)
            {
                ...
                if (!_hasDoCancel)
                {
                    [self gtcPerformTouch:_touchCancelTarget withAction:_touchCancelAction];
...
-(void)gtcPerformTouch:(id)target withAction:(SEL)action
{
    if (_layout != nil && target != nil && action != nil && self.gtcActionSender != nil)
    {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [target performSelector:action withObject:[self gtcActionSender]];
#pragma clang diagnostic pop
...
@implementation GTCLayoutTouchEventDelegate
{
