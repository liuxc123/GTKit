...
static inline UIColor *GTCProgressViewDefaultTintColor(void) {
...
static const CGFloat GTCProgressViewTrackColorDesaturation = 0.3f;

static const NSTimeInterval GTCProgressViewAnimationDuration = 0.25;

@interface GTCProgressView ()
...
@implementation GTCProgressView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self commonGTCProgressViewInit];
    }
    return self;
}
...
- (void)commonGTCProgressViewInit {
    self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    self.backgroundColor = [UIColor clearColor];
    self.clipsToBounds = YES;
    self.isAccessibilityElement = YES;

    _backwardProgressAnimationMode = GTCProgressViewBackwardAnimationModeReset;
    ...
    _progressView.backgroundColor = GTCProgressViewDefaultTintColor();
...
- (void)setProgressTintColor:(UIColor *)progressTintColor {
    if (progressTintColor == nil) {
        progressTintColor = GTCProgressViewDefaultTintColor();
    }
...
- (void)setProgress:(float)progress
    ...
    if (progress < self.progress &&
        self.backwardProgressAnimationMode == GTCProgressViewBackwardAnimationModeReset) {
        self.progress = 0;
        [self updateProgressView];
    }
...
- (void)accessibilityValueDidChange {
    ...
    // deallocate self.
    GTCProgressView *strongSelf = self;
...
+ (NSTimeInterval)animationDuration {
    return GTCProgressViewAnimationDuration;
}
...
+ (UIColor *)defaultTrackTintColorForProgressTintColor:(UIColor *)progressTintColor {
    ...
    if ([progressTintColor getHue:&hue saturation:&saturation brightness:&brightness alpha:&alpha]) {
        CGFloat newSaturation = MIN(saturation * GTCProgressViewTrackColorDesaturation, 1.0f);
...
- (void)updateProgressView {
    ...
    CGFloat progressWidth = GTCCeil(self.progress * CGRectGetWidth(self.bounds));
