...
typedef NS_ENUM(NSInteger, GTCProgressViewBackwardAnimationMode) {

    /** Animate negative progress by resetting to 0 and then animating to the new value. */
    GTCProgressViewBackwardAnimationModeReset,

    /** Animate negative progress by animating from the current value. */
    GTCProgressViewBackwardAnimationModeAnimate
};
 ...
 */
IB_DESIGNABLE
@interface GTCProgressView : UIView
...
/**
 ...
 will determine which animation to use. The default is GTCProgressViewBackwardAnimationModeReset.
 */
@property(nonatomic, assign) GTCProgressViewBackwardAnimationMode backwardProgressAnimationMode;
